// Self-Learning ML Service Demo (Fixed)
...
async function runSelfLearningDemo() {
  ...
  // Train models
  const modelsToTrain = ['code-analysis', 'code-smell-detection'];
  ...
  for (const analysisType of ['complexity', 'code-smell']) {
    try {
  ...
  const modelPerformanceValues = Object.values(stats.modelPerformance);
  const avgAccuracy = modelPerformanceValues.length > 0 
    ? (modelPerformanceValues.reduce((sum, p) => sum + p.accuracy, 0) / modelPerformanceValues.length * 100).toFixed(1)
    : '0.0';
