 ...
 * 
 * Run this script with Node.js to diagnose PATH issues.
 * Usage: "C:\Program Files\nodejs\node.exe" scripts/diagnose-path.js
 * 
 * If even that doesn't work, open the script output manually:
 * 1. Open PowerShell or Command Prompt from Start Menu
 * 2. Navigate to this project: cd E:\Self Built Web and Mobile Apps\Space Analyzer
 * 3. Run: node scripts/diagnose-path.js
 */
...
for (const exe of criticalExes) {
  ...
  if (!found) {
    ...
    // Search for it
    for (const driveLetter of "CDEFGH".split("")) {
      const searchPath = `${driveLetter}:\\Program Files\\nodejs\\node.exe`;
      if (exe.name === "node.exe" && fs.existsSync(searchPath)) {
...
// 4. Check system PATH (machine-level, not user-level)
console.log("🔴 SYSTEM PATH ANALYSIS:");
console.log("-".repeat(50));

// Check if System32 is in the path at all
...
if (missingCritical.length === 0) {
  ...
  console.log();
  console.log("  Option A (Run as Administrator):");
  console.log('    setx PATH "C:\\Windows\\System32;C:\\Windows;%PATH%" /M');
  console.log();
  console.log("  Option B (Use the fix script in this project):");
  console.log("    Right-click 'scripts/fix-path-manual.cmd' → Run as Administrator");
  console.log("    Then restart Visual Studio Code");
  console.log();
  console.log("  Option C (Manual via System Properties):");
  ...
  console.log("    6. Click OK, restart VS Code");
}
