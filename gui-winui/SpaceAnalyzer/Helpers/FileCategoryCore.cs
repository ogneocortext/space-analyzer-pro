...
/// <summary>
/// WinUI-free file-extension → storage-category classification. Mirrors the Rust
/// scanner's <c>extension_to_category</c> mapping (and the canonical
/// <c>FILE_CATEGORIES</c> in the main crate) so the WinUI history detail,
/// contract tests, and every other consumer classify files identically.
///
/// Kept free of Windows/WinUI types (no <c>Microsoft.UI.*</c>) so it can be
/// linked into the headless unit-test project.
/// </summary>
public static class FileCategoryCore
{
    ...
    public static readonly Dictionary<string, (byte R, byte G, byte B)> CategoryColors = new(StringComparer.OrdinalIgnoreCase)
    {
