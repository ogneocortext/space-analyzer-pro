...
### Rust CLI — `ask` Command & Agentic Tools (2026-08-19)

- **New `ask` subcommand** — `space-analyzer-cli ask "<question>" [--scan-id N]` lets the local Ollama model answer free-form questions about a **saved** scan using read-only tool-calling. It reconstructs the `ScanReport` from the embedded DB (latest scan, or `--scan-id`) so the agent reasons over real data **without re-scanning the disk**. Mirrors the existing `scan --ask` agentic loop (model auto-selection, 5 tool rounds, prompt cache) but is decoupled from the live scan path.
- **Three new non-destructive agentic tools** in `tool_registry` (all read-only, same safety tier as the existing history/disk tools):
  - `classify_file` — origin-trace/classify a path via `origin_tracer::assess_file` (returns origin category, deletion-safety verdict, owning-app presence, and related files). Always available.
  - `get_bloat_findings` — runs the `offline_ai` classifier over the scan's top directories + largest files and returns reclaimable categories with priority scores. Scan-dependent.
  - `get_recommendations` — returns the prioritized cleanup-recommendation list for the scan. Scan-dependent.
- **Recommendation engine moved into the core crate** (`src/recommendations.rs`) so the `tool_registry` (lib) can call it without a binary→lib→binary cycle. `cli::render` and `cli::types::Recommendation` now re-export the core definitions, so `scan --cleanup-recommendations`, `recommend`, and the WinUI dashboard are unchanged in behavior — they simply share the rule engine with the agent. `cli::recommend` reconstructs the report via the new `ScanReport::from_history_record` helper.
