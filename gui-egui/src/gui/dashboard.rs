    ...
    pub(crate) fn render_dashboard(&mut self, ui: &mut egui::Ui) {
        ...
        if is_compact {
            ...
            ui.columns(2, |cols| {
                ...
                left.add_space(10.0);
                self.render_disk_space_live(left);
                left.add_space(10.0);
    ...
    fn render_file_type_chart(&self, ui: &mut egui::Ui) {
        ...
        app_card(ui, |ui| {
            if let Some(ref result) = self.scan_result {
                ...
                let top_n: Vec<_> = sorted.iter().take(10).collect();

                if top_n.is_empty() {
                    ui.label(
                        egui::RichText::new("No file types in scan results")
                            .italics()
                            .color(colors::TEXT_MUTED),
                    );
                } else {
                    // Chart with thicker line, grid, and point markers
                    let bar_points: PlotPoints = top_n
                        .iter()
                        .enumerate()
                        .map(|(i, (_ext, count))| [i as f64, **count as f64])
                        .collect();
                    ...
                    let plot = Plot::new("file_type_dist")
                        .height(150.0)
                        .show_grid([true, true])
                    ...
                    ui.add_space(4.0);
                    ui.horizontal_wrapped(|ui| {
                        for (ext, count) in top_n.iter() {
                            let (r, g, b) = category::category_color(ext);
                            ui.colored_label(
                ...
                if total == 0 {
                    ...
                    for (cat, count) in sorted.iter().take(8) {
                        ...
                        ui.vertical(|ui| {
                            ...
                            gauge_bar(ui, bar_pct, ui.available_width(), 6.0);
                        });
                        ui.add_space(4.0);
                    }
                }
        ...
        card_frame(ui.style()).show(ui, |ui| {
            ...
            if snapshots.is_empty() {
                ui.vertical_centered(|ui| {
                    ui.add_space(16.0);
                    ui.label(
                        egui::RichText::new("Monitoring disk space...")
                            .size(13.0)
                            .color(colors::TEXT_MUTED),
                    );
                    ui.label(
                        egui::RichText::new("Snapshots appear every 5 seconds")
                            .size(11.0)
                            .color(colors::TEXT_MUTED),
                    );
                    ui.add_space(12.0);
            ...
            let total_gb = latest.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0
                + latest.available_bytes as f64 / 1024.0 / 1024.0 / 1024.0;
            let used_gb = latest.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0;
            let free_gb = latest.available_bytes as f64 / 1024.0 / 1024.0 / 1024.0;

            // Big stats row
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new(format!("{:.1} GB", used_gb))
                        .size(18.0)
                        .strong()
                        .color(colors::TEXT_PRIMARY),
                );
                ...
                ui.add_space(12.0);
                ui.label(
                    egui::RichText::new(format!("{:.1} GB", free_gb))
                        .size(18.0)
                        .strong()
                        .color(colors::SUCCESS),
                );
                ...
                ui.add_space(12.0);
                ui.label(
                    egui::RichText::new(format!("{:.1} GB total", total_gb))
                        .size(11.0)
                        .color(colors::TEXT_SECONDARY),
                );
            });
            ...
            let fill_color = if latest.usage_percent > 90.0 {
                colors::ERROR
            } else if latest.usage_percent > 75.0 {
                colors::WARNING
            } else {
                colors::SUCCESS
            };
            ui.painter().rect_filled(rect, 5.0, colors::SURFACE_3);
            let fill_rect = egui::Rect::from_min_size(
                rect.min,
                egui::vec2(bar_width * (latest.usage_percent / 100.0), bar_height),
            );
            ui.painter().rect_filled(fill_rect, 5.0, fill_color);
            // Percentage text on bar
            ui.painter().text(
                rect.left_center()
                    + egui::vec2(bar_width * (latest.usage_percent / 100.0) / 2.0, 0.0),
                egui::Align2::CENTER_CENTER,
                format!("{:.1}%", latest.usage_percent),
            ...
            Plot::new("disk_live_chart")
                .height(plot_height)
                .show_grid([true, true])
                .x_axis_label("Time (snapshots ago)")
                .y_axis_label("Used (GB)")
                .legend(Legend::default())
                .coordinates_formatter(
                    Corner::LeftTop,
                    CoordinatesFormatter::new(|point, _value| {
                        let gb = point.y;
                        let ago = point.x as i64;
                        format!("{} GB  |  {} snapshots ago", gb, ago)
                    }),
                )
                .show(ui, |plot_ui| {
                    // Compute Y range for better auto-scaling
                    let min_y = snapshots
                        .iter()
                        .map(|s| s.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0)
                        .fold(f64::INFINITY, f64::min);
                    let max_y = snapshots
                        .iter()
                        .map(|s| s.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0)
                        .fold(0.0_f64, f64::max);
                    let y_margin = ((max_y - min_y) * 0.15).max(0.5);
                    plot_ui.set_plot_bounds(egui_plot::PlotBounds::from_min_max(
                        [0.0, min_y - y_margin],
                        [snapshots.len().saturating_sub(1) as f64, max_y + y_margin],
                    ));

                    let points: PlotPoints = snapshots
                        .iter()
                        .enumerate()
                        .map(|(i, s)| [i as f64, s.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0])
                        .collect();

                    let line = Line::new("Used GB", points)
                        .color(colors::ACCENT)
                        .width(2.5)
                        .fill_alpha(0.08);
                    plot_ui.line(line);
                    ...
                    if let Some(ref change) = self.disk_monitor.last_change {
                        if let Ok(change_dt) =
                            chrono::DateTime::parse_from_rfc3339(&change.timestamp)
                        {
                            if let Some(idx) = snapshots.iter().position(|s| {
                                chrono::DateTime::parse_from_rfc3339(&s.timestamp)
                                    .map(|dt| {
                                        (dt.timestamp() - change_dt.timestamp()).unsigned_abs() < 6
                                    })
                                    .unwrap_or(false)
                            }) {
                                let y = snapshots[idx].used_bytes as f64 / 1024.0 / 1024.0 / 1024.0;
                                let sign = if change.delta_bytes > 0 { "+" } else { "" };
                                let mb = (change.delta_bytes as f64 / 1024.0 / 1024.0).abs();
                                plot_ui.points(
                                    egui_plot::Points::new(
                                        format!("{}{:.0}MB", sign, mb),
                                        egui_plot::PlotPoints::from(vec![[idx as f64, y]]),
                                    )
            ...
            if let Some(ref change) = self.disk_monitor.last_change {
                ...
                if !change.top_processes.is_empty() {
                    ui.add_space(4.0);
                    ui.label(
                        egui::RichText::new("Top processes at that moment:")
                            .size(11.0)
                            .color(colors::TEXT_MUTED),
                    );
                    for proc in change.top_processes.iter().take(3) {
                        let io_mb =
                            (proc.disk_read_bytes + proc.disk_write_bytes) as f64 / 1024.0 / 1024.0;
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new(format!("  {} (pid {})", proc.name, proc.pid))
                                    .size(11.0)
                                    .color(colors::TEXT_SECONDARY),
                            );
                            ui.label(
                                egui::RichText::new(format!("{:.1}MB I/O", io_mb))
                                    .size(11.0)
                                    .color(colors::TEXT_MUTED),
                            );
                        });
            ...
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new(format!("{} snapshots recorded", snapshots.len()))
                    .size(10.0)
                    .color(colors::TEXT_MUTED),
            );
        });
