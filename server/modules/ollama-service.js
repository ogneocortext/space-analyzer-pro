...
async function checkOllamaAvailability(enhancedOllama, modelPerformance) {
    try {
        ...
        if (isAvailable) {
            console.log('🦙 Enhanced Ollama is available - AI features enhanced with production features');
            
            // Load models using enhanced service
            const models = await enhancedOllama.fetchModels();
            this.ollamaModels = models.map(m => m.name);
            console.log(`📚 Loaded ${this.ollamaModels.length} Enhanced Ollama models:`, this.ollamaModels.join(', '));
