...
class HardwareDetector {
  ...
  detectCPU() {
    ...
    if (process.platform === 'win32') {
      try {
        const wmicOutput = execSync('wmic cpu get NumberOfLogicalProcessors,NumberOfCores,Name /format:csv 2>nul', 
    ...
    const isHighEnd = /(i9|ryzen 9|threadripper|xeon.*(gold|platinum)|epyc|m3 max|m2 max|m3 pro|m2 pro)/i.test(model);
    const isMidRange = /(i7|i5|ryzen 7|ryzen 5|xeon|m1|m2|m3)/i.test(model) && !isHighEnd;
    const isLowEnd = /(i3|ryzen 3|pentium|celeron|atom)/i.test(model) || (!isHighEnd && !isMidRange && cpuCount <= 4);
  ...
  detectMemory() {
    ...
    if (process.platform === 'win32') {
      try {
        const wmicOutput = execSync('wmic ComputerSystem get TotalPhysicalMemory /format:csv 2>nul', 
          { encoding: 'utf8', timeout: 3000, windowsHide: true });
        const lines = wmicOutput.trim().split('\n').filter(l => l.includes(','));
        if (lines.length > 0) {
          const parts = lines[lines.length - 1].split(',');
  ...
  generateRecommendations() {
    ...
    if (!selectedModel) {
      selectedModel = gpu.hasHighEndGPU ? 'qwen2.5:14b' : gpu.hasGPU ? 'qwen2.5:7b' : 'phi4-mini:latest';
    }
