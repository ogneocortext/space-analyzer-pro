    ...
    async loadOllamaModels() {
        try {
            ...
            if (models && models.models) {
                ...
                console.log(`📚 Loaded ${this.ollamaModels.length} Ollama models:`, this.ollamaModels.join(', '));
    ...
    setupRoutes() {
        ...
        this.app.post('/api/ai/insights', async (req, res) => {
            ...
            try {
                ...
                const cachedResponse = await this.checkSelfLearningCache(query, analysisData, selectedModel);
                if (cachedResponse) {
                    this.recordInteraction(query, analysisData, selectedModel, cachedResponse, Date.now() - startTime, true);
                    return res.json({
                        success: true,
                ...
                await this.storeSelfLearningResponse(query, analysisData, selectedModel, ollamaResponse.response);

                // Record interaction for learning
                this.recordInteraction(query, analysisData, selectedModel, ollamaResponse.response, Date.now() - startTime, true);
                ...
                this.recordInteraction(query || '', analysisData || {}, this.currentModel, null, Date.now() - startTime, false);
        ...
        this.app.post('/api/analyze', async (req, res) => {
            try {
                ...
                const dirName = path.basename(resolvedPath).replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 30);
                const now = new Date();
                ...
                try {
                    ...
                    if (previousAnalysis && previousAnalysis.analysis_data) {
                        console.log(`📊 Found previous analysis (${new Date(previousAnalysis.last_analyzed).toISOString()})`);
                        ...
                        console.log(`🔄 File changes detected: +${changes.added.length} ~${changes.changed.length} -${changes.removed.length} (total: ${totalChanges})`);

                        // If minimal changes (< 5% of files), reuse previous analysis with updates
                        const changeThreshold = Math.max(10, Math.floor(previousAnalysis.analysis_data.totalFiles * 0.05));
                        if (totalChanges < changeThreshold) {
                            ...
                            // Update timestamps
                            incrementalResult.analysisTime = Date.now() - Date.parse(previousAnalysis.last_analyzed);
                            ...
                            console.log(`🔄 Significant changes detected (${totalChanges} >= ${changeThreshold}), running full analysis`);
                        }
                    }
                } catch (incrementalError) {
                    console.log('⚠️ Incremental analysis check failed, proceeding with full analysis:', incrementalError.message);
                }

                if (incrementalResult) {
                    // Return incremental result
                    const normalizedPath = directoryPath.startsWith('./') ? directoryPath.substring(2) : directoryPath;
        ...
        this.app.post('/api/files/open-explorer', async (req, res) => {
            try {
                if (process.platform === 'win32') {
                    ...
                    const command = process.platform === 'darwin' ? `open "${homePath}"` : `xdg-open "${homePath}"`;
                    spawn(command, { shell: true });
                ...
                for (const file of files) {
                    if (file.endsWith('.json')) {
                        ...
                        analyses.push({
                            ...
                            date: data.timestamp || new Date(file.replace('analysis_', '').replace('.json', '')).toISOString(),
        ...
        this.app.post('/api/ai-models/qa', async (req, res) => {
            ...
            try {
                ...
                await this.knowledgeDB.storeAIResponse(question, ollamaResponse.response, contextHash, model, responseTime);
...
Largest Files:
${(analysisData.large_files || []).slice(0, 5).map(f => `- ${f}`).join('\n') || 'None detected'}
...
`;
    ...
    categorizeFile(filename) {
        ...
        const categories = {
            ...
            // Code
            'js': 'Code', 'ts': 'Code', 'tsx': 'Code', 'jsx': 'Code', 'py': 'Code', 'java': 'Code', 'cpp': 'Code', 'c': 'Code', 'cs': 'Code', 'go': 'Code', 'rs': 'Code',
            // System
            'exe': 'System', 'dll': 'System', 'sys': 'System', 'bat': 'System', 'sh': 'System', 'ps1': 'System'
        };
...
Answer:`;
    ...
    generateAIInsights(rustData) {
        ...
        const commonExtensions = ['js', 'ts', 'py', 'java', 'cpp', 'c', 'h', 'html', 'css', 'json', 'md', 'txt', 'png', 'jpg', 'gif'];
        ...
        if (rustData.categories) {
            ...
            if (!categories.includes('Code') && categories.some(c => c.toLowerCase().includes('src') || c.toLowerCase().includes('code'))) {
                insights.recommended_categories.push('Consider creating a "Code" category for source files');
            }
        }
        ...
        const depFiles = rustData.files
            .filter(f => ['package.json', 'requirements.txt', 'Pipfile', 'pyproject.toml', 'Cargo.toml', 'go.mod'].includes(f.name))
            .map(f => f.path);
        ...
        if (rustData.files && rustData.files.some(f => f.path.includes('node_modules'))) {
            insights.optimization_suggestions.push('node_modules detected - can be safely excluded from backups');
        }
        if (rustData.files && rustData.files.some(f => f.name.endsWith('.zip'))) {
            insights.optimization_suggestions.push('ZIP archives found - consider extracting for better organization');
        }
    ...
    async convertRustOutputToWebFormat(rustData, analysisId, isMlSummary = false) {
        if (isMlSummary) {
            ...
            return {
                ...
                categories: this.convertCategoryDistribution(rustData.ml_summary?.features?.category_distribution || {}),
                extensionStats: this.convertExtensionDistribution(rustData.ml_summary?.features?.extension_distribution || {}),
        ...
        return {
            ...
            mlFeatures: {
                ...
                complexity_metrics: {
                    ...
                    extension_diversity: Object.keys(rustData.extension_stats || rustData.extensionStats || {}).length
                }
            }
        };
    }
