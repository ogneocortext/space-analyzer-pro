...
class AnalysisController {
  ...
  getAnalysisHistory = asyncHandler(async (req, res) => {
    ...
    // In a real implementation, this would fetch from a database
    // For now, return empty array as mock response
    res.json({
      success: true,
      analyses: [],
      total: 0,
      ...
      message: "No analysis history available. This is a mock response.",
    });
  });
  ...
  generateEnhancedInsights(scanResult) {
    ...
    const largeFiles = scanResult.files
      ? scanResult.files.filter((f) => f.size > 100 * 1024 * 1024)
      : [];
    ...
    const oldFiles = scanResult.files
      ? scanResult.files.filter((f) => f.modified && f.modified < oneYearAgo)
      : [];
    ...
    const mediaFiles = scanResult.files
      ? scanResult.files.filter(
          (f) => f.category === "Images" || f.category === "Videos" || f.category === "Audio"
        )
      : [];
    if (mediaFiles.length > totalFiles * 0.5) {
      insights.push({
        ...
        aiRecommendation:
          "AI suggests implementing media compression and CDN for better performance",
      });
    }

    // 6. Code files insight (enhanced)
    const codeFiles = scanResult.files
      ? scanResult.files.filter((f) => f.category === "Code" || f.category === "Web Development")
      : [];
    ...
    const compressibleFiles = scanResult.files
      ? scanResult.files.filter((f) => compressibleExtensions.includes(f.extension))
      : [];
    const compressibleSize = compressibleFiles.reduce((sum, f) => sum + f.size, 0);

    if (compressibleSize > totalSize * 0.1) {
      // > 10% of total size
      insights.push({
  ...
  generateRecommendations(scanResult) {
    ...
    if (oldFiles.length > totalFiles * 0.2) {
      // > 20% are old
      recommendations.push({
        type: "cleanup",
        priority: "high",
    ...
    if (totalSize > 1024 * 1024 * 1024) {
      ...
      recommendations.push({
        ...
        aiOptimization:
          "AI suggests implementing tiered storage with hot/warm/cold data classification",
      });
    }
    ...
    if (codeFiles.length > 0) {
      recommendations.push({
        ...
        aiRecommendation:
          "AI suggests implementing automated backup scheduling and code quality monitoring",
      });
    }
    ...
    if (scannerStats && scannerStats.activeScans > 3) {
      recommendations.push({
        ...
        aiMonitoring:
          "AI system will automatically optimize scan scheduling based on resource availability",
      });
    }
