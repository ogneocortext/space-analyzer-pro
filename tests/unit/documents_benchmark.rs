...
fn benchmark_documents_folder_through_cli() {
    ...
    eprintln!(
        "{:<16} {:>6} {:>6} {:>10} {:>10} {:>8}",
        "Mode", "Files", "Dirs", "Size", "Time", "Files/s"
    );
    eprintln!("{:-<58}", "");
    ...
    for (label, flags) in cases {
        ...
        cmd.args(["scan", "--path", path, "--format", "json"])
            .args(&flags);
        ...
        let dirs = json
            .get("total_dirs")
            .and_then(|v| v.as_u64())
            .or_else(|| json.get("total_directories").and_then(|v| v.as_u64()))
            .unwrap_or(0);
        let size = json
        ...
        if let Some(prev) = last_files {
            assert!(
                files >= prev,
                "deeper scan ({label}) found fewer files than shallower scan"
            );
        }
