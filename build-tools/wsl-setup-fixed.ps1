# Fixed WSL Setup Script for Space Analyzer Pro
...
try {
    ...
    if ($vmPlatform.State -eq "Disabled" -or $wslFeature.State -eq "Disabled") {
        Write-Host ""
        Write-Host "WSL features are still disabled"
        Write-Host "Please restart your computer after enabling features"
        exit 1
    }
} catch {
...
if ($wslWorking) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "SUCCESS! Space Analyzer Pro is now running in WSL!"
