    ...
    AnalysisResult analyzeDirectory(const std::string& path) {
        ...
        try {
            ...
            if (config.enableParallel) {
                analyzeDirectoryParallel(path);
            } else {
                scanDirectorySequential(path, 0);
            }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Security validation of directory path
        if (directory.length() > 260) {
            std::cerr << "Error: Directory path too long (max 260 characters)\n";
            return 1;
        }
        
        // Check for dangerous path characters
        const std::string dangerousChars = "<>:"|?*";
        if (directory.find_first_of(dangerousChars) != std::string::npos) {
            std::cerr << "Error: Directory path contains invalid characters\n";
            return 1;
        }
