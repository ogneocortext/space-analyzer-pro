...
export class SelectionManager {
  ...
  public selectNodesInArea(startPoint: THREE.Vector2, endPoint: THREE.Vector2, nodes: FileNode[]): void {
  ...
  // Select by Raycast
  public selectByRaycast(mousePosition: THREE.Vector2, nodes: FileNode[], addToSelection: boolean = false): FileNode | null {
  ...
  public selectBySizeRange(nodes: FileNode[], minSize: number, maxSize: number): void {
    const nodesInRange = nodes.filter(node => 
      node.size >= minSize && node.size <= maxSize
    )
  ...
  public selectByNamePattern(nodes: FileNode[], pattern: string, useRegex: boolean = false): void {
    ...
    try {
      regex = useRegex ? new RegExp(pattern, 'i') : new RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
    } catch (error) {
...
class AreaSelection {
  ...
  public containsPoint(point: THREE.Vector2): boolean {
    return point.x >= this.minBounds.x &&
