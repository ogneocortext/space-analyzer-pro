...
export class ComparisonManager {
  ...
  private createDiffView(): void {
    ...
    const differentNodes = [
      ...this.comparisonResult.different,
      ...this.comparisonResult.onlyInLeft,
      ...this.comparisonResult.onlyInRight,
    ];
    ...
    differentNodes.forEach((node, index) => {
      ...
      this.createNodeVisualization(
        ...
        this.getStatusColor(node.comparisonStatus)
      );
    });
  }
  ...
  private createDirectoryVisualization(
    directory: FileNode,
    position: THREE.Vector3,
    side: "left" | "right",
    opacity: number = 1
    ...
    if (directory.children) {
      directory.children.forEach((child, index) => {
        ...
        this.createNodeVisualization(
          child,
          childPosition,
          this.getStatusColor(child.comparisonStatus),
          opacity
        );
      });
    }
  ...
  private createNodeVisualization(
    node: FileNode,
    position: THREE.Vector3,
    color: number,
    opacity: number = 1
  ...
  public getDetailedComparison(): {
    fileTypes: Record<
      string,
      { left: number; right: number; identical: number; different: number }
    >;
