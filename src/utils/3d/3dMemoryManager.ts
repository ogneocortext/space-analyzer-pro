...
export class ThreeDMemoryManager {
  ...
  private checkMemoryUsage(): void {
    ...
    console.log(
      `Memory usage: ${this.formatBytes(this.currentMemoryUsage)} / ${this.formatBytes(this.maxMemoryUsage)}`
    );
  }
...
class ObjectPool {
  ...
  public optimize(): void {
    ...
    while (this.pool.length > targetSize) {
      ...
      if (
        obj &&
        (this.type === "geometry" || this.type === "material" || this.type === "texture")
      ) {
        obj.dispose?.();
      }
    }
  ...
  public getStats(): any {
    return {
      ...
      hitRate:
        this.acquireCount > 0 ? (this.acquireCount - this.createCount) / this.acquireCount : 0,
    };
  }
...
export class GeometryPoolManager {
  ...
  public static returnSphereGeometry(
  ...
  // Return Box Geometry
  public static returnBoxGeometry(
