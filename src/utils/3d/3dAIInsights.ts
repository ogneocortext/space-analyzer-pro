...
export class AIInsightsManager {
  ...
  private filterAndRankInsights(insights: AIInsight[]): AIInsight[] {
    ...
    const filtered = insights.filter(
      (insight) => insight.confidence >= 0.5 && insight.priority !== InsightPriority.LOW
    );
...
class UsagePatternAnalyzer implements InsightGenerator {
  async generateInsights(context: AnalysisContext): Promise<AIInsight[]> {
    ...
    const neglectedFiles = usageData
      .filter(
        (data) =>
          data.accessPattern === "declining" ||
          data.lastAccessed < new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
      )
  ...
  async generateInsights(context: AnalysisContext): Promise<AIInsight[]> {
    ...
    if (duplicates.length > 0) {
      const totalDuplicateSize = duplicates.reduce(
        (sum, group) => sum + group.slice(1).reduce((groupSum, file) => groupSum + file.size, 0),
        0
      );
...
class SecurityAnalyzer implements InsightGenerator {
  ...
  private findSensitiveFiles(fileSystem: FileNode): FileNode[] {
    ...
    const sensitivePatterns = [/password/i, /secret/i, /key/i, /credential/i, /token/i, /private/i];

    const traverse = (node: FileNode) => {
      if (node.type === "file") {
        const isSensitive = sensitivePatterns.some(
          (pattern) => pattern.test(node.name) || pattern.test(node.path)
        );
        if (isSensitive) {
