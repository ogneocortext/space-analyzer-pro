...
export class Settings3DManager {
  ...
  private listeners: Map<string, ((category: string, key: string, value: any) => void)[]>
  ...
  // Add Listener
  public addListener(callback: (category: string, key: string, value: any) => void): void {
  ...
  public getPerformanceProfile(): 'low' | 'medium' | 'high' | 'ultra' {
    const { maxMemoryMB, targetFPS, enableLOD, enableFrustumCulling, enableObjectPooling } = this.settings.performance
    
    if (maxMemoryMB >= 1024 && targetFPS >= 120 && enableLOD && enableFrustumCulling && enableObjectPooling) {
      return 'ultra'
  ...
  public getSettingsSummary(): {
    ...
    Object.entries(this.settings).forEach(([category, categorySettings]) => {
      ...
      Object.keys(categorySettings).forEach(key => {
        ...
        if (categorySettings[key as keyof typeof categorySettings] !== 
            DEFAULT_SETTINGS[category as keyof Settings3D][key as keyof typeof DEFAULT_SETTINGS[keyof typeof Settings3D]]) {
          modifiedSettings++
        }
      })
    })
...
export class SettingsValidator {
  ...
  public static sanitizeValue(category: string, key: string, value: any): any {
