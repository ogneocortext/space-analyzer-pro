...
export function usePerformanceMonitor() {
  ...
  const startMonitoring = () => {
    ...
    renderMonitor = window.setInterval(() => {
      ...
      const frameTime = now - (metrics.value.timestamp as number);
      
      metrics.value.render.frameTime = frameTime;
      metrics.value.timestamp = new Date(now);
      
      checkThresholds('render', frameTime, thresholds.value.render);
    }, 100);
    ...
    networkMonitor = window.setInterval(() => {
      ...
      checkThresholds('network', latency, { warning: 150, critical: 300 });
    }, 5000);
   ...
   */
  const generateOverallRecommendations = (score: number, grade: string): string[] => {
   ...
   */
  const updateRenderMetrics = (fps: number, frameTime: number, drawCalls: number, triangles: number) => {
