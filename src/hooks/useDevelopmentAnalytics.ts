...
export const useDevelopmentAnalytics = (projectPath: string) => {
  const detectCodeDuplication = useCallback(
    async (files: string[]): Promise<CodeDuplicationResult> => {
  ...
  const trackBuildPerformance = useCallback(async (): Promise<BuildPerformanceMetrics> => {
    ...
    const averageBuildTime =
      buildHistory.reduce((sum, build) => sum + build.buildTime, 0) / buildHistory.length;
    const averageBundleSize =
      buildHistory.reduce((sum, build) => sum + build.bundleSize, 0) / buildHistory.length;
  ...
  const getOptimizationScore = useCallback((metrics: BuildPerformanceMetrics): number => {
    ...
    const bundleSizeScore = Math.max(
      0,
      100 - (metrics.averageBundleSize / (100 * 1024 * 1024)) * 100
    ); // Lower bundle size is better
