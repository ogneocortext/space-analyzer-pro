...
export const useAnalysisProgress = (analysisId: string | null) => {
  ...
  const [scannedFiles, setScannedFiles] = useState<
    Array<{ name: string; path: string; size: number; category: string }>
  >([]);
  ...
  useEffect(() => {
    ...
    const currentAnalysisId =
      analysisId || (typeof window !== "undefined" ? (window as any).__currentAnalysisId : null);

    console.warn("🔍 useAnalysisProgress - analysisId:", analysisId);
    ...
    if (
      lastMessage &&
      lastMessage.type === "file_scanned" &&
      lastMessage.analysisId === currentAnalysisId
    ) {
      console.warn("📁 File scanned:", lastMessage.file);
    ...
    if (
      lastMessage &&
      lastMessage.type === "analysis_complete" &&
      lastMessage.analysisId === currentAnalysisId
    ) {
      console.warn("✅ Analysis complete message received");
      setProgress((prev) => ({
        ...prev,
        percentage: 100,
        completed: true,
