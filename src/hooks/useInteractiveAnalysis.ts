...
export const useInteractiveAnalysis = (options: UseInteractiveAnalysisOptions = {}) => {
  ...
  const [quickAnalysisCache, setQuickAnalysisCache] = useState<Map<string, QuickAnalysisResult>>(new Map());
  ...
  const performQuickAnalysis = useCallback(async (path: string): Promise<QuickAnalysisResult> => {
    // Check cache first
  ...
  const startAnalysis = useCallback(async (path: string, analysisOptions: {
    ...
    try {
      ...
      const stepDelay = analysisOptions.speed === 'fast' ? 50 : 
                       analysisOptions.speed === 'slow' ? 200 : 100;
      ...
      for (let step = 0; step <= totalSteps; step++) {
        ...
        setAnalysisState(prev => ({
          ...prev,
          progress,
          status: step < 25 ? '🔍 Scanning directory structure...' :
                  step < 75 ? '🤖 Running AI categorization...' :
                  step < 90 ? '💡 Generating AI recommendations...' :
      ...
      const finalResults = {
        ...
        performance: {
          filesPerSecond: analysisOptions.speed === 'fast' ? 100 : 
                        analysisOptions.speed === 'slow' ? 25 : 50,
  ...
  const executeAction = useCallback(async (actionId: string, params: any = {}) => {
    if (!options.onExecuteAction) {
      console.log('Mock action execution:', actionId, params);
      return;
    }
  ...
  const changeSpeed = useCallback((speed: 'slow' | 'normal' | 'fast') => {
    setAnalysisState(prev => ({
      ...prev,
      currentSpeed: speed
    }));
