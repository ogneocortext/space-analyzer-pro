...
export class DependencyChecker {
    constructor() {
        ...
        this.dependencyPatterns = {
            ...
            java: [
                /import\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)\s*;/g
            ],
            cpp: [
                /#include\s*[<"]([^>"]+)[>"]/g
            ],
    ...
    async analyzeDependencies(files) {
        ...
        dependencyGraph.missingDependencies = await this.detectMissingDependencies(dependencyGraph.nodes, dependencyGraph.edges);
    ...
    findDependencyNode(dep, nodes) {
        ...
        if (!node) {
            ...
            node = nodes.find(n =>
                n.name.toLowerCase().includes(depLower) ||
                depLower.includes(n.name.toLowerCase())
            );
        }
