...
const AIContextProviderInternal: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const initializeAICapabilities = async () => {
            try {
                ...
                console.log('🧠 AI Capabilities initialized:', capabilities);
            } catch (error) {
    ...
    return (
        <AIContextInternal.Provider value={value}>
            {children}
        </AIContextInternal.Provider>
    );
};
...
export const useAIActions = () => {
    ...
    return {
        ...
        updateAIStatus: (status: any) => 
            dispatch({ type: 'UPDATE_AI_STATUS', payload: status }),
