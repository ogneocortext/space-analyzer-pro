...
export class ABTestingFramework {
  ...
  private async calculateTestResults(test: ABTest): Promise<ABTestResults> {
    ...
    for (const variant of test.variants) {
      ...
      const result = await this.calculateVariantResults(variant, variantEvents, test.successMetrics)
  ...
  private determineWinner(variantResults: VariantResult[], significance: number): string | undefined {
  ...
  private generateInsights(variantResults: VariantResult[], test: ABTest): string[] {
    ...
    if (sorted.length >= 2) {
      ...
      insights.push(`${sorted[0].variantId} shows ${improvement.toFixed(1)}% better conversion rate`)
    }
  ...
  private generateRecommendations(variantResults: VariantResult[], test: ABTest, winner?: string): string[] {
