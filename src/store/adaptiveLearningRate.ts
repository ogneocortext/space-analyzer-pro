...
export class AdaptiveLearningRate {
  ...
  private async calculateBehaviorMetrics(
    ...
    return {
      ...
      successRate: await this.calculateSuccessRate(recommendations),
    };
  }
  ...
  private calculateExplorationRate(events: UsageEvent[]): number {
    ...
    const uniquePaths = new Set(
      events.filter((e) => e.type === "directory-navigation" && e.data.path).map((e) => e.data.path)
    );
  ...
  private async calculateFeedbackQuality(recommendations: Recommendation[]): Promise<number> {
    ...
    const averageRating =
      recentFeedback.reduce((sum, r) => sum + (r.feedback?.rating || 3), 0) / recentFeedback.length;
  ...
  private async calculateSuccessRate(recommendations: Recommendation[]): Promise<number> {
    const feedbackRecords = await indexedDBPersistence.loadAnalyticsData("user-feedback", 1000);
    ...
    const successfulFeedback = relevantFeedback.filter((r) => r.feedback?.rating >= 4);
  ...
  private shouldAdjustRate(behaviorChange: number): boolean {
    ...
    return (
      behaviorChange > this.config.adjustmentThreshold &&
  ...
  private updateAdaptiveParameters(metrics: BehaviorMetrics): void {
    ...
    if (metrics.consistencyScore > 0.8) {
      this.currentParameters.patternDetectionThreshold = Math.max(
        0.3,
        this.currentParameters.patternDetectionThreshold - 0.05
      );
    } else if (metrics.consistencyScore < 0.4) {
      this.currentParameters.patternDetectionThreshold = Math.min(
        0.8,
        this.currentParameters.patternDetectionThreshold + 0.05
      );
    }
    ...
    if (metrics.successRate > 0.8) {
      this.currentParameters.recommendationWeight = Math.min(
        1.0,
        this.currentParameters.recommendationWeight + 0.1
      );
    } else if (metrics.successRate < 0.5) {
      this.currentParameters.recommendationWeight = Math.max(
        0.1,
        this.currentParameters.recommendationWeight - 0.1
      );
    }
    ...
    if (metrics.feedbackQuality > 0.8) {
      this.currentParameters.feedbackInfluence = Math.min(
        1.0,
    ...
    if (metrics.explorationRate < 0.2) {
      this.currentParameters.explorationBonus = Math.min(
        0.5,
        this.currentParameters.explorationBonus + 0.1
      );
    } else if (metrics.explorationRate > 0.8) {
      this.currentParameters.explorationBonus = Math.max(
        0,
  ...
  async loadHistory(): Promise<void> {
    try {
      const records = await indexedDBPersistence.loadAnalyticsData(
