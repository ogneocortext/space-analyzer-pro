...
impl SpaceAnalyzerApp {
    ...
    pub(crate) fn render_scan(&mut self, ui: &mut egui::Ui) {
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let stop_btn = egui::Button::new(egui::RichText::new("Stop").size(13.0).strong())
                    .fill(colors::ERROR)
                    .corner_radius(egui::CornerRadius::same(8))
    ...
    fn show_largest_files(&mut self, ui: &mut egui::Ui, result: &ScanResult) {
        ...
        ui.horizontal(|ui| {
            ...
            if !self.largest_files_filter.is_empty() && ui.small_button("Clear").clicked() {
        ...
        egui::Grid::new("largest_files_grid")
            ...
            .show(ui, |ui| {
                ...
                ui.end_row();
                for (path, size) in filtered_files {
                    ui.label(
