use super::*;
use egui_plot::{PlotPoints, Line, Plot, Legend, CoordinatesFormatter, Corner, HLine};
...
impl SpaceAnalyzerApp {
    pub(crate) fn render_dashboard(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            if let Some(ref result) = self.scan_result {
                ui.label(egui::RichText::new(format!("Last scan: {} files, {}",
                    result.total_files, formatting::format_bytes(result.total_size_bytes))).strong());
            } else {
            ...
            if let Some(ref result) = self.scan_result {
                let threshold = (self.settings.large_file_threshold_mb as u64) * 1024 * 1024;
                let large_count = result.largest_files.iter()
                    .filter(|(_, size)| *size > threshold)
                    .count();
                if large_count > 0 {
                    ui.separator();
                    ui.label(egui::RichText::new(format!("Large files (>{}MB): {}",
        ...
        // ── Quick Actions ───────────────────────────────────────────
        ui.horizontal(|ui| {
            if ui.add_enabled(!self.is_scanning, egui::Button::new("📁 Scan Now")).clicked() {
        ...
        if let Some(ref sys) = self.system_resources {
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.add(egui::ProgressBar::new(sys.cpu_percent / 100.0)
                ...
                ui.vertical(|ui| {
                    ...
                    ui.add(egui::ProgressBar::new(sys.memory_percent / 100.0)
                        .text(format!("{:.1}% ({}/{})",
                            sys.memory_percent,
        ...
        if !self.disk_volumes.is_empty() {
            ...
            for vol in &self.disk_volumes {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add(egui::ProgressBar::new(vol.usage_percent / 100.0)
                        .text(format!("{:.1}% — {} free of {}",
                            vol.usage_percent,
        ...
        if !self.ai_recommendations.is_empty() {
            ui.separator();
            let source_label = if self.ai_recommendation_source == "ai" { "🤖 AI" } else { "⚙ Heuristic" };
            ui.horizontal(|ui| {
                ui.heading("Insights");
        ...
        egui::CollapsingHeader::new(
            egui::RichText::new("AI Assistant").strong())
            .default_open(true)
            .show(ui, |ui| {
        ...
        egui::CollapsingHeader::new(
            egui::RichText::new("Smart Search & Embeddings").strong())
            .default_open(false)
            .show(ui, |ui| {
        ...
        egui::CollapsingHeader::new(
            egui::RichText::new("AI Conversation").strong())
            .default_open(false)
            .show(ui, |ui| {
        ...
        egui::CollapsingHeader::new(
            egui::RichText::new("System & Settings").strong())
            .default_open(false)
            .show(ui, |ui| {
    ...
    fn render_ai_status(&self, ui: &mut egui::Ui) {
        if self.ollama_available {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Connected").color(egui::Color32::GREEN).strong());
            ...
            if !self.discovered_models.is_empty() {
                ui.small(format!("{} models discovered", self.discovered_models.len()));
            }
    ...
    fn render_activity_status(&mut self, ui: &mut egui::Ui) {
        ...
        if self.is_scanning {
            has_activity = true;
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("● Scanning").color(egui::Color32::YELLOW).strong());
        ...
        if self.is_indexing {
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(egui::ProgressBar::new(self.indexing_progress / 100.0)
        ...
        if let Some(ref execution) = self.active_workflow {
            ...
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new(format!("Workflow: {}", execution.workflow_name))
                    .color(status_color).strong());
                if execution.status == ExecutionStatus::Running {
                    ui.label(format!("{}/{}", execution.actions_completed, execution.total_actions));
                }
            });
        }
        ...
        if !self.pending_workflow_actions.is_empty() {
            has_activity = true;
            ui.small(format!("{} pending actions", self.pending_workflow_actions.len()));
        }

        if !has_activity {
            ui.label(egui::RichText::new("No active processes").italics().color(egui::Color32::GRAY));
        }
    }
    ...
    fn render_smart_search_status(&self, ui: &mut egui::Ui) {
        if self.cached_embeddings.is_empty() {
            ...
            ui.horizontal(|ui| {
                ui.label(format!("{} embeddings indexed", self.cached_embeddings.len()));
                if let Some(scan_id) = self.embedding_scan_id {
                    ui.small(format!("(scan #{}))", scan_id));
                }
            });
            if !self.search_results.is_empty() {
                ui.label(format!("Last search: {} results", self.search_results.len()));
            }
    ...
    fn render_conversation_status(&self, ui: &mut egui::Ui) {
        if self.chat_messages.is_empty() {
            ...
            ui.horizontal(|ui| {
                ...
                ui.separator();
                ui.label(format!("{} turns of context", self.conversation_history.len()));
                ui.separator();
    ...
    fn render_storage_chart(&self, ui: &mut egui::Ui) {
        ...
        let timestamps: Vec<chrono::DateTime<chrono::Utc>> = self.scan_history.iter()
        ...
        let size_points: PlotPoints = self.scan_history.iter().enumerate().map(|(i, r)| {
            let x = if use_dates { timestamps[i].timestamp() as f64 } else { i as f64 };
            [x, r.total_size_mb]
        }).collect();
        ...
        let max_files = self.scan_history.iter().map(|r| r.total_files as f64).fold(0.0_f64, f64::max);
        let max_size = self.scan_history.iter().map(|r| r.total_size_mb).fold(0.0_f64, f64::max);

        let file_points: PlotPoints = self.scan_history.iter().enumerate().map(|(i, r)| {
            let x = if use_dates { timestamps[i].timestamp() as f64 } else { i as f64 };
        ...
        let mut plot = Plot::new("storage_trend")
            ...
            .coordinates_formatter(Corner::LeftTop, CoordinatesFormatter::new(|point, _value| {
                if use_dates {
        ...
        if use_dates { plot = plot.x_axis_label("Date"); }
        else { plot = plot.x_axis_label("Scan #"); }
        ...
        plot.show(ui, |plot_ui| {
            ...
            if threshold_mb > 0.0 {
                plot_ui.hline(HLine::new("Large file threshold", threshold_mb)
        ...
        ui.horizontal(|ui| {
            if let Some(latest) = self.scan_history.first() {
                ui.small(format!("Latest: {} files, {:.2} MB",
