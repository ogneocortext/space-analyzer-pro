...
impl SpaceAnalyzerApp {
    ...
    fn render_gpu_settings(&mut self, ui: &mut egui::Ui) {
        ...
        if self.settings.gpu_acceleration {
            ui.indent("gpu_options", |ui| {
                ui.checkbox(&mut self.settings.cuda_enabled, "Enable CUDA Kernels (requires CUDA toolkit at compile time)");
                ui.checkbox(&mut self.settings.dedup_use_gpu, "Use GPU for Deduplication Hashing");
    ...
    fn render_logging_settings(&mut self, ui: &mut egui::Ui) {
        ...
        if self.settings.log_session_to_file {
            ui.indent("log_options", |ui| {
                ...
                ui.small("Logs all user interactions for automated flow testing and issue detection.");
            });
        }
    }
    ...
    pub(crate) fn render_settings(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.button("Save Settings").clicked() {
            ...
            if (self.settings.ollama_url != old_url || self.settings.ollama_model != old_model || self.settings.ollama_enabled != old_enabled)
