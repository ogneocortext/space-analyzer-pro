export interface AnalysisResult {
    ...
    categories?: { [key: string]: { count: number, size: number, files: Array<{ name: string, size: number, path: string }> } };
    ...
    dependencyGraph?: {
        nodes: Array<{ id: string, name: string, path: string, size: number, type: string, extension?: string }>;
...
export class AnalysisBridge {
    ...
    private initializeBaseUrl() {
        try {
            ...
            const backendApiUrl = typeof import.meta !== 'undefined' ? import.meta.env.VITE_BACKEND_API_URL : undefined;
            ...
            if (backendApiUrl) {
                ...
                if (!serverBackendUrl) {
                    throw new Error('BACKEND_API_URL environment variable is required for server-side operation');
                }
            ...
            throw new Error(`Failed to initialize AnalysisBridge: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async analyzeDirectoryWithProgress(
        ...
        console.log('🔍 Starting analysis (backend will generate ID)');
        ...
        const analyzeResponse = await this.fetchWithRetry(`${this.baseUrl}/analyze`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        }, 300000, 5); // 5 minute timeout, 5 retries for analysis

        console.log('📡 Analysis request response status:', analyzeResponse.status);
        ...
        const immediateAnalysisId = startResponse.analysisId || startResponse.serviceMetadata?.analysisId;
        console.log('📋 Analysis ID for progress tracking:', immediateAnalysisId);
        ...
        if (startResponse.success && startResponse.result && startResponse.result.summary) {
            console.log('✅ Results returned directly in start response');
        ...
        if (onProgress) {
            console.log('🚀 Starting progress polling with callback...');
            let pollAttempts = 0;
            ...
            const pollProgress = async () => {
                try {
                    pollAttempts++;
                    console.log(`🔄 Progress poll attempt ${pollAttempts}/${maxPollAttempts}`);
                    ...
                    if (progressResponse.ok) {
                        ...
                        if (progressData.success && progressData.progress) {
                            ...
                            console.log('📊 About to call progress callback with:', {
                            ...
                            console.log('✅ Progress callback called successfully');
                        }
                        
                        // Continue polling if not complete (reduced from 1s to 2s for better performance)
                        if (progressData.progress && progressData.progress.status !== 'complete' && pollAttempts < maxPollAttempts) {
            ...
            console.log('🚀 Starting pollProgress immediately...');
            pollProgress();
        } else {
        ...
        while (attempts < maxAttempts) {
            console.log(`🔄 Results poll attempt ${attempts + 1}/${maxAttempts}`);
    ...
    private generateCategoriesFromSummary(backendResult: { strategy?: string; summary?: { totalFiles: number; totalSize: number } }): { [key: string]: { count: number, size: number, files: Array<{ name: string, size: number, path: string }> } } {
        const categories: { [key: string]: { count: number, size: number, files: Array<{ name: string, size: number, path: string }> } } = {};

        // Generate some basic categories based on the directory type and strategy
        if (backendResult.strategy === 'media-focused') {
            categories['Media Files'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.4), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.6), files: [] };
            categories['Documents'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.3), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.2), files: [] };
            categories['System Files'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.2), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.15), files: [] };
            categories['Other'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.1), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.05), files: [] };
        } else {
            // Generic categorization
            categories['Code Files'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.25), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.2), files: [] };
            categories['Documents'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.25), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.3), files: [] };
            categories['Media'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.2), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.4), files: [] };
            categories['System'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.15), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.05), files: [] };
            categories['Other'] = { count: Math.floor((backendResult.summary?.totalFiles || 0) * 0.15), size: Math.floor((backendResult.summary?.totalSize || 0) * 0.05), files: [] };
        }
    ...
    async searchFiles(
        ...
        files: Array<{ name: string, size: number, path: string, extension: string, directory: string }>;
        pagination: {
            page: number;
            pageSize: number;
            total: number;
        ...
        const response = await fetch(`${this.baseUrl}/files/search`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ directory, query, page, pageSize, sortBy, sortOrder, filterExtension })
        });
    ...
    async selectDirectory(): Promise<string | null> {
        if ('showDirectoryPicker' in window) {
            try {
                ...
                (window as unknown as { selectedDirectoryHandle?: { name: string } }).selectedDirectoryHandle = handle;
    ...
    async exportData(data: Record<string, unknown>, format: string = 'json', filename: string = 'analysis'): Promise<{ success: boolean; filename: string; path: string; size: number }> {
    ...
    async getOllamaInsights(analysisData: Record<string, unknown>, query?: string): Promise<Record<string, unknown>> {
    ...
    async getLearningTrends(directory: string, days: number = 30): Promise<Record<string, unknown>> {
        const response = await fetch(`${this.baseUrl}/learning/trends/${encodeURIComponent(directory)}?days=${days}`);
        if (!response.ok) throw new Error('Failed to get learning trends');
        const data = await response.json();
        return data.trends;
    }

    async getLearningChanges(directory: string, days: number = 7): Promise<{
        added: number;
        modified: number;
        deleted: number;
        netChange: number;
    }> {
        const response = await fetch(`${this.baseUrl}/learning/changes/${encodeURIComponent(directory)}?days=${days}`);
    ...
    // NLP Methods
    async nlpSearch(query: string, directory: string, options?: {
        page?: number;
        pageSize?: number;
        limit?: number;
    }): Promise<{
    ...
    async nlpSuggestions(prefix: string, limit: number = 10): Promise<string[]> {
        const response = await fetch(`${this.baseUrl}/nlp/suggestions/${encodeURIComponent(prefix)}?limit=${limit}`);
    ...
    async aiModelQA(question: string, directory: string, analysisId?: string): Promise<{
        success: boolean;
        question: string;
        answer: string;
        intent: string;
    ...
    async categorizeFilesOptimized(directoryPath: string, options: Record<string, unknown> = {}): Promise<{
        success: boolean;
        results: {
    ...
    async getAIRecommendations(directoryPath: string, analysisData?: Record<string, unknown>): Promise<{
        success: boolean;
    ...
    async advancedSearch(query: string, directoryPath: string, options: Record<string, unknown> = {}): Promise<{
        success: boolean;
        results: Array<{
            name: string;
            size: number;
    ...
    async findDuplicates(directoryPath: string, options: Record<string, unknown> = {}): Promise<{
        success: boolean;
        duplicates: Array<{
            group: string;
            files: Array<{
