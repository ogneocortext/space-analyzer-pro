        ...
        switch (type) {
            ...
            case ErrorType.ANALYSIS:
                if (context?.path) {
                    actions.push({
                        ...
                        action: () => {
                            const input = document.querySelector('input[placeholder*="directory path"]') as HTMLInputElement;
                            if (input) {
                                input.focus();
                                input.select();
                            }
            ...
            case ErrorType.TIMEOUT:
                ...
                actions.push({
                    ...
                    action: () => alert('For very large directories, the analysis may take longer. Try again.')
                });
                break;
            ...
            case ErrorType.PERMISSION:
                ...
                actions.push({
                    ...
                    action: () => alert('Ensure you have read access to the target directory and its contents.')
                });
                break;
    ...
    static formatErrorMessage(error: any): { type: ErrorType; message: string; details?: string } {
        ...
        if (errorMessage.includes('fetch') || errorMessage.includes('network') || errorMessage.includes('connection')) {
            return {
                ...
                details: 'Unable to connect to the Space Analyzer backend. Please check if the server is running.'
            };
        }
        ...
        // Validation errors
        if (errorMessage.includes('path') || errorMessage.includes('directory') || errorMessage.includes('not found')) {
            return {
