...
export class LayerAnalyzer {
    ...
    const violations = graph.links
      ...
      .map(link => {
        ...
        return {
          ...
          violation: sourceNode.layer > targetNode.layer ? 'downward' as const : 'upward' as const
        };
      });
  ...
  getLayerStatistics(layerAnalysis: LayerAnalysis): {
    ...
    return {
      ...
      violationsByType: layerAnalysis.violations.reduce((acc, v) => {
        acc[v.violation] = (acc[v.violation] || 0) + 1;
        return acc;
      }, {} as { [key: string]: number }),
      nodesPerLayer: layerAnalysis.layers.reduce((acc, l) => {
  ...
  getMostProblematicLayer(layerAnalysis: LayerAnalysis): {
    ...
    layerAnalysis.violations.forEach(violation => {
      violationCounts.set(violation.sourceLayer, (violationCounts.get(violation.sourceLayer) || 0) + 1);
    });
