...
export class StatisticsCalculator {
  ...
  getHighImpactSuggestions(
    ...
    return suggestions.filter((suggestion) => {
      const totalImpact =
        suggestion.impact.complexityReduction +
  ...
  getPriorityScore(suggestion: OptimizationSuggestion): number {
    ...
    const effortPenalty = suggestion.effort === "high" ? 5 : suggestion.effort === "medium" ? 2 : 0;

    const riskPenalty = suggestion.risk === "high" ? 3 : suggestion.risk === "medium" ? 1 : 0;
