...
export class LayerCalculator {
  calculateLayers(nodes: DependencyNode[], links: DependencyLink[]): void {
    console.log('📊 Calculating dependency layers...');
    
    // Find root nodes (no dependencies)
    const rootNodes = nodes.filter(node => node.dependencies.length === 0);
    
    // BFS to assign layers
    const queue: { node: DependencyNode; layer: number }[] = rootNodes.map(node => ({ node, layer: 0 }));
    ...
    nodes.forEach(node => {
      if (!visited.has(node.id)) {
        node.layer = Math.max(...node.dependencies.map(depId => {
          const depNode = nodes.find(n => n.id === depId);
