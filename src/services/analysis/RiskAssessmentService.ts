...
export class RiskAssessmentService {
  ...
  private initializeRiskModels(): void {
    console.warn("🔮 Initializing risk assessment models...");
    ...
    console.warn(`✅ Initialized ${this.riskModels.size} risk assessment models`);
  }

  // Load historical data
  private loadHistoricalData(): void {
    console.warn("📊 Loading historical risk data...");
    ...
    console.warn("✅ Historical data loaded");
  }
  ...
  async assessChangeRisk(changeImpact: ChangeImpact, context: any = {}): Promise<ChangeRisk> {
    console.warn("🔮 Assessing risk for code changes...");
    ...
    console.warn(`✅ Risk assessment completed in ${duration}ms`);
    console.warn(`📊 Overall risk: ${overallRisk.toFixed(1)}% (${riskLevel})`);
  ...
  private async assessPerformanceRisk(
    changeImpact: ChangeImpact,
    context: any
  ): Promise<RiskFactor> {
  ...
  private calculateCodeChurn(changeImpact: ChangeImpact): number {
    ...
    return Math.min(
      100,
      (changeImpact.linesAdded + changeImpact.linesRemoved + changeImpact.linesModified) * 0.2
    );
  }
  ...
  public getRiskTrends(days: number = 30): any {
    ...
    const averageRisk =
      recentRisks.reduce((sum, risk) => sum + risk.overallRisk, 0) / recentRisks.length;
