...
export class MultiObjectiveCodeSmellDetector {
  ...
  async detectCodeSmellsMultiObjective(
    code: string,
    filePath: string
  ): Promise<MultiObjectiveResult> {
    console.warn(`🔍 Multi-objective code smell detection for ${filePath}`);
  ...
  private async detectDesignSmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
  ...
  private async detectImplementationSmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
    const smells: DetectedCodeSmell[] = [];
  ...
  private async detectNamingSmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
    const smells: DetectedCodeSmell[] = [];
    ...
    namingIssues.forEach((issue, index) => {
      smells.push({
        ...
        suggestion:
          "Follow consistent naming convention (camelCase for variables, PascalCase for classes)",
  ...
  private async detectDocumentationSmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
    const smells: DetectedCodeSmell[] = [];
  ...
  private async detectPerformanceSmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
    const smells: DetectedCodeSmell[] = [];
  ...
  private async detectSecuritySmells(
    code: string,
    metrics: CodeMetrics
  ): Promise<DetectedCodeSmell[]> {
    const smells: DetectedCodeSmell[] = [];
  ...
  private calculateOverallScore(smells: DetectedCodeSmell[]): number {
    ...
    const maxWeight = smells.reduce(
      (sum, smell) => sum + this.getSeverityWeight(smell.severity),
      0
    );
  ...
  private calculateCategoryScores(smells: DetectedCodeSmell[]): { [category: string]: number } {
    ...
    categories.forEach((category) => {
      ...
      if (categorySmells.length === 0) {
        ...
        const avgConfidence =
          categorySmells.reduce((sum, s) => sum + s.confidence, 0) / categorySmells.length;
    ...
    sortedCategories.forEach(([category, score]) => {
      if (score < 70) {
        recommendations.push(
          `Focus on improving ${category} - current score: ${score.toFixed(1)}%`
        );
      }
    });
  ...
  private initializeDetectionModels(): void {
    ...
    console.warn("🧠 Initializing multi-objective detection models...");
  }
