...
const _DEPENDENCY_PATTERNS = {
  ...
  cpp: [/#include\s*[<"]([^>"]+)[>"]/g, /#pragma\s+once/g],
...
export class FileDependencyParser {
  ...
  private static async extractDependencies(file: {
    name: string;
    path: string;
    size: number;
  }): Promise<string[]> {
  ...
  private static calculateStats(nodes: FileNode[], edges: DependencyEdge[]) {
    ...
    nodes.forEach((node, index) => {
      ...
      const dependencyComplexity = Math.min(
        1,
        (node.dependencies.length + node.dependents.length) / 10
      );
