import { FileDependency, DependencyImpact, PredictiveDependencyAnalysis } from './types/dependency-types';
...
export class DependencyCheckerService {
  ...
  async analyzeActionImpact(action: string, targetFiles: string[]): Promise<PredictiveDependencyAnalysis> {
    console.log(`🎯 Analyzing impact of ${action} on ${targetFiles.length} files`);
    ...
    for (const directImpact of directImpacts) {
      for (const affectedFile of directImpact.affectedFiles) {
        if (!processedFiles.has(affectedFile)) {
          const cascadingImpact = await this.analyzeCascadingImpact(action, affectedFile, directImpact);
  ...
  private async analyzeCascadingImpact(action: string, affectedFile: string, directImpact: DependencyImpact): Promise<DependencyImpact> {
  ...
  private calculateImpactLevel(action: string, targetFile: string, affectedFiles: string[], _isCascading = false): { level: 'critical' | 'high' | 'medium' | 'low' | 'none', reason: string, risk: 'critical' | 'high' | 'medium' | 'low' } {
  ...
  private generateConsequences(action: string, _targetFile: string, affectedFiles: string[]): string[] {
  ...
  private generateCascadingConsequences(action: string, _affectedFile: string, directImpact: DependencyImpact): string[] {
    const consequences: string[] = [];
  ...
  private calculateOverallRisk(directImpacts: DependencyImpact[], cascadingImpacts: DependencyImpact[]): 'critical' | 'high' | 'medium' | 'low' {
  ...
  private generateRecommendations(action: string, directImpacts: DependencyImpact[], cascadingImpacts: DependencyImpact[]): string[] {
    const recommendations: string[] = [];
    
    const criticalImpacts = [...directImpacts, ...cascadingImpacts].filter(i => i.riskLevel === 'critical');
    const highImpacts = [...directImpacts, ...cascadingImpacts].filter(i => i.riskLevel === 'high');
  ...
  private generateAlternativeActions(action: string, _targetFiles: string[]): { action: string; risk: 'critical' | 'high' | 'medium' | 'low'; benefits: string[]; description: string }[] {
    const alternatives: { action: string; risk: 'critical' | 'high' | 'medium' | 'low'; benefits: string[]; description: string }[] = [];
  ...
  private escalateRisk(risk: 'critical' | 'high' | 'medium' | 'low'): 'critical' | 'high' | 'medium' | 'low' {
    const riskLevels = ['low', 'medium', 'high', 'critical'];
    const currentIndex = riskLevels.indexOf(risk);
    return riskLevels[Math.min(currentIndex + 1, riskLevels.length - 1)] as any;
  }

  private deescalateRisk(risk: 'critical' | 'high' | 'medium' | 'low'): 'critical' | 'high' | 'medium' | 'low' {
    const riskLevels = ['low', 'medium', 'high', 'critical'];
    const currentIndex = riskLevels.indexOf(risk);
  ...
  private async analyzeJavaScriptFile(filePath: string, content: string): Promise<CodeAnalysisResult> {
    ...
    // Parse imports
    const importRegex = new RegExp('import\\s+(?:(?:\\*\\s+as\\s+(\\w+)|(\\w+)\\s*,\\s*\\{([^}]+)\\}|(\\w+)|\\{([^}]+)\\})\\s+from\\s+[\'"]([^\'"]+)[\'"]|require\\s*\\([\'"]([^\'"]+)[\'"]\\))', 'g');
    let match;
    ...
    // Parse exports
    const exportRegex = /export\s+(?:(?:default\s+(?:(?:class|function|const|let|var)\s+(\w+)|\{([^}]+)\})|(?:class|function|const|let|var)\s+(\w+))|(\{[^}]+\})\s*from\s+['"]([^'"]+)['"])/g;
    ...
    // Parse functions
    const functionRegex = new RegExp('(?:function\\s+(\\w+)|const\\s+(\\w+)\\s*=\\s*(?:function\\s*\\([^)]*\\)|\\([^)]*\\)\\s*=>)|(\\w+)\\s*\\([^)]*\\)\\s*\\{', 'g');
    ...
    while ((match = variableRegex.exec(content)) !== null) {
      ...
      const varType = declaration.includes('const') ? 'const' : 
                     declaration.includes('let') ? 'let' : 'var';
  ...
  private async analyzeUsage(
    ...
    for (const variable of variables) {
      ...
      variable.isUsed = regex.test(content.replace(new RegExp(`(?:const|let|var)\\s+${variable.name}\\s*=`), ''));
    }
    
    // Mark functions as used
    for (const func of functions) {
      const regex = new RegExp(`\\b${func.name}\\s*\\(`, 'g');
      func.isUsed = regex.test(content.replace(new RegExp(`function\\s+${func.name}\\s*\\(|const\\s+${func.name}\\s*=\\s*function`), ''));
    }
  ...
  async traceFrontendBackendConnections(files: any[]): Promise<{
    ...
    const missingConnections = await this.findMissingConnections(frontendComponents, backendEndpoints);
