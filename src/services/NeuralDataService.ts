...
export class NeuralDataService {
   ...
   */
  private createConnections(nodes: NeuralData['nodes'], categories: any): NeuralData['connections'] {
    ...
    categoryNodes.forEach((node1, i) => {
      categoryNodes.slice(i + 1).forEach(node2 => {
        const strength = this.calculateCategorySimilarity(node1.category!, node2.category!, categories);
        if (strength > 0.3) {
          connections.push({
            from: node1.id,
            to: node2.id,
   ...
   */
  private calculateMetrics(nodes: NeuralData['nodes'], connections: NeuralData['connections']): NeuralData['metrics'] {
  ...
  private getFileCategory(file: any, categories: any): string {
    ...
    const categoryMap: { [key: string]: string } = {
      ...
      '.pdf': 'documents', '.doc': 'documents', '.txt': 'documents', '.md': 'documents',
      '.zip': 'archives', '.rar': 'archives', '.7z': 'archives',
      '.exe': 'system', '.dll': 'system', '.sys': 'system'
    };
  ...
  private calculateAnomalyScore(nodes: NeuralData['nodes'], connections: NeuralData['connections']): number {
    ...
    const avgConnections = nodes.length > 0 
      ? Array.from(connectionCounts.values()).reduce((sum, count) => sum + count, 0) / nodes.length 
      : 0;
