...
export class EnhancedSelfLearningService extends EventEmitter {
  ...
  constructor(config: Partial<IncrementalLearningConfig> = {}) {
    ...
    console.log('🧠 Enhanced Self-Learning Service initialized');
  }

  // Initialize learning triggers
  private initializeTriggers(): void {
    console.log('🔧 Initializing learning triggers...');
    ...
    console.log(`✅ Initialized ${this.learningTriggers.length} learning triggers`);
  }
  ...
  private async evaluateTrigger(trigger: LearningTrigger, result: AnalysisResult): Promise<boolean> {
    switch (trigger.type) {
      case 'file-change':
  ...
  private async executeTriggerAction(trigger: LearningTrigger): Promise<void> {
    console.log(`🚀 Executing trigger action: ${trigger.action} (priority: ${trigger.priority})`);
  ...
  private async performIncrementalTraining(): Promise<void> {
    ...
    if (dbStats.totalSamples < this.config.minSamplesForRetraining) {
      console.log(`⏸️ Insufficient samples for retraining: ${dbStats.totalSamples} < ${this.config.minSamplesForRetraining}`);
      return;
    }
    ...
    for (const modelName of modelsToTrain) {
      ...
      if (!performance || performance.accuracy < 0.8) {
        try {
          ...
          console.log(`✅ Incremental training completed for model: ${modelName}`);
        } catch (error) {
  ...
  private async evaluateAndTrain(): Promise<void> {
    console.log('🔍 Evaluating and training...');
    ...
    for (const [modelName, metrics] of Object.entries(evaluation)) {
      if (metrics.accuracy < 0.75) {
        try {
          await this.mlService.trainModel(modelName);
          console.log(`✅ Evaluation-driven training completed for model: ${modelName}`);
        } catch (error) {
  ...
  private async performFeedbackDrivenTraining(): Promise<void> {
    console.log('👥 Performing feedback-driven training...');
    
    if (this.feedbackBuffer.length === 0) {
      console.log('⏸️ No feedback available for training');
      return;
    }
    ...
    try {
      ...
      console.log('✅ Feedback-driven training completed');
    } catch (error) {
  ...
  private async performScheduledTraining(): Promise<void> {
    console.log('⏰ Performing scheduled training...');
    
    const dbStats = this.mlService.getTrainingDatabaseStats();
    console.log(`📊 Training database stats: ${dbStats.totalSamples} total samples`);
    ...
    // Train all models
    const models = ['code-analysis', 'code-smell-detection', 'refactoring-suggestion', 'pattern-recognition'];
    
    for (const modelName of models) {
      try {
        await this.mlService.trainModel(modelName);
        console.log(`✅ Scheduled training completed for model: ${modelName}`);
      } catch (error) {
  ...
  private async performPerformanceDrivenTraining(): Promise<void> {
    ...
    for (const modelName of degradedModels) {
      try {
        await this.mlService.trainModel(modelName);
        console.log(`✅ Performance-driven training completed for model: ${modelName}`);
      } catch (error) {
  ...
  private handleTrainingCompleted(data: any): void {
    console.log(`🎉 Training completed for model: ${data.modelName}`);
  ...
  async predictWithLearning(
    ...
    // Get learning info
    const feedback = this.feedbackBuffer.filter(f => 
  ...
  getKnowledgeInsights(): {
    ...
    if (knowledgeBase['code-smells']) {
      ...
      recommendations.push(`Most common code smells: ${topSmells.map(([name]) => name).join(', ')}`);
    }
