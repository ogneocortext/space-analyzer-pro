  ...
  private async initializeParsers() {
    ...
    for (const lang of languages) {
      try {
        ...
        console.warn(`🔧 Loaded parser for ${lang}`);
      } catch (error) {
...
export class EnhancedCodeAnalysisService {
  ...
  private findUnusedImports(
    ...
    return imports.filter((imp) => {
      ...
      const isUsed = usageNodes.some(
        (node) => node.text === imp.localName || node.text === imp.name
      );
      imp.isUsed = isUsed;
      return !isUsed;
    });
  ...
  private calculateComplexity(ast: any): number {
    ...
    const decisionNodes = ast.rootNode.descendantsOfType([
      "if_statement",
      "while_statement",
      "for_statement",
      "switch_statement",
  ...
  async warmCache(files: any[]) {
    console.warn("🔥 Warming up cache with frequently used files...");
    ...
    console.warn(`✅ Cache warmed with ${priorityFiles.length} files`);
  }
}
