...
export class LearningService {
    ...
    trackAnalysis(directoryPath: string, analysisResult: any): void {
        ...
        try {
            const analysisHistory = JSON.parse(localStorage.getItem('spaceAnalyzer_analysisHistory') || '[]');
    ...
    getUsageStats(): {
        ...
        const favoriteView = Object.entries(viewCounts)
            .sort(([,a], [,b]) => b - a)[0]?.[0] || 'dashboard';
        ...
        this.actions.forEach(action => {
            ...
            if (actionTime - lastActionTime > 30 * 60 * 1000) { // 30 minutes
                sessions++;
            }
            lastActionTime = actionTime;
        });

        const totalTime = this.actions.length > 1
            ? this.actions[this.actions.length - 1].timestamp.getTime() - this.actions[0].timestamp.getTime()
            : 0;
