...
export interface CodeIssue {
  ...
  line: number;
  column: number;
  type: 'missing_dependency' | 'unused_variable' | 'unused_import' | 'undefined_variable' | 'dead_code' | 'potential_bug';
...
export class CodeAnalysisService {
  ...
  private analyzeJavaScriptDependencies(filePath: string, content: string): DependencyHealthAnalysis {
    const analysis: DependencyHealthAnalysis = {
      missingDependencies: [],
      unusedImports: [],
      unusedVariables: [],
    ...
    const unusedVars = Array.from(definedVariables).filter(varName => !usedVariables.has(varName));
  ...
  private analyzePythonFile(filePath: string, content: string): CodeIssue[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (importMatch) {
        const moduleName = importMatch[2];
        // Simple check - in real implementation would be more sophisticated
        if (!content.includes(moduleName, content.indexOf(importMatch[0]) + importMatch[0].length)) {
          issues.push({
            file: filePath,
            line: index + 1,
  ...
  private calculateHealthScore(totalFiles: number, totalIssues: number, issuesBySeverity: { [key: string]: number }): number {
  ...
  private generateHealthRecommendations(issuesByType: { [key: string]: number }, issuesBySeverity: { [key: string]: number }): string[] {
