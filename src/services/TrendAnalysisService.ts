...
export class TrendAnalysisService {
  ...
  private metrics: string[] = ['complexity', 'maintainability', 'testCoverage', 'technicalDebt', 'issues', 'codeChurn', 'security', 'performance'];
  ...
  private initializeForecastModels(): void {
    console.log('📈 Initializing forecast models...');
    ...
    console.log(`✅ Initialized ${this.metrics.length} forecast models`);
  }

  // Load historical data
  private loadHistoricalData(): void {
    console.log('📊 Loading historical trend data...');
  ...
  async analyzeTrends(period: string = '30d'): Promise<TrendAnalysis> {
    ...
    console.log(`✅ Trend analysis completed for ${period}`);
  ...
  private generateAlerts(trends: any): string[] {
    ...
    Object.entries(trends).forEach(([metric, trend]: [string, any]) => {
      ...
      if (trend.trend === 'degrading' && trend.changePercent < -20) {
        alerts.push(`Alert: ${metric} is rapidly declining (${trend.changePercent.toFixed(1)}% change)`);
      }
      ...
      if (trend.forecast && trend.forecast.length > 0) {
        ...
        if (lastForecast < 30) {
          alerts.push(`Forecast: ${metric} expected to be critically low (${lastForecast.toFixed(1)}%)`);
  ...
  async getComprehensiveAnalysis(): Promise<any> {
    console.log('📈 Generating comprehensive trend analysis...');
  ...
  private exportToCSV(): string {
    const headers = ['timestamp', 'complexity', 'maintainability', 'testCoverage', 'technicalDebt', 'issues', 'codeChurn', 'security', 'performance'];
