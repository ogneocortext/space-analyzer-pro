...
export class ExportService {
  ...
  static exportToCSV(data: AnalysisResult, filename?: string): void {
    ...
    // Build CSV content
    const headers = ['Name', 'Size (bytes)', 'Size (KB)', 'Size (MB)', 'Extension', 'Category', 'Path'];
  ...
  static exportToTextReport(data: AnalysisResult, filename?: string): void {
    ...
    if (data.categories) {
      ...
      for (const [category, info] of sortedCategories) {
        ...
        lines.push(`${category.padEnd(20)} ${info.count.toString().padStart(8)} files  ${sizeMB.padStart(12)} MB`);
      }
      lines.push('');
    }
    ...
    if (data.extensionStats) {
      ...
      for (const [ext, info] of sortedExtensions) {
        const sizeMB = (info.size / 1024 / 1024).toFixed(2);
        lines.push(`.${ext.padEnd(10)} ${info.count.toString().padStart(8)} files  ${sizeMB.padStart(12)} MB`);
      }
      lines.push('');
    }
   ...
   */
  static exportFileManifest(files: Array<{ name: string; path: string; size: number }>, filename?: string): void {
    const manifest = {
