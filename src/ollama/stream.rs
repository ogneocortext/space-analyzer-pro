    ...
    Box::pin(stream.then(|chunk| {
        async move {
            ...
            let text = match String::from_utf8(chunk.to_vec()) {
                Ok(t) => t,
                Err(e) => return futures::stream::iter(vec![Err(OllamaError::ParseError(format!("Invalid UTF-8 in stream: {}", e)))]),
            };
            ...
            for line in text.lines() {
                ...
                match serde_json::from_str::<StreamResponse>(line) {
                    ...
                    Err(e) => {
                        results.push(Err(OllamaError::ParseError(format!("Failed to parse stream chunk: {}", e))));
                    }
                }
            }
