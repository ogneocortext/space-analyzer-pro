<template>
  <div v-if="store.isAnalysisRunning" class="flex items-center gap-3">
    ...
    <div class="relative" ref="dropdownRef">
      ...
      <div 
        v-if="dropdownOpen"
        class="absolute right-0 top-full mt-2 w-80 bg-slate-900 border border-slate-700 rounded-lg shadow-xl z-50"
        @click.stop
      >
        <div class="p-4 border-b border-slate-700">
          ...
          <div class="space-y-2">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-xs font-medium text-white">{{ store.progressData.files?.toLocaleString() || 0 }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-slate-400">Current File:</span>
              <span class="text-xs font-medium text-white truncate max-w-[150px]" :title="store.progressData.currentFile">
                {{ getFileName(store.progressData.currentFile) }}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-slate-400">Path:</span>
              <span class="text-xs font-medium text-white truncate max-w-[150px]" :title="store.path">
