  ...
  <div class="space-y-4 lg:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div class="flex items-center space-x-3 min-w-0 flex-1">
        <BrainCircuit class="w-5 h-5 sm:w-6 sm:h-6 text-purple-400 shrink-0" />
    ...
    >
      <div class="bg-gray-800 rounded-lg p-3 sm:p-4">
        <div class="flex items-center justify-between">
          ...
          <BarChart3 class="w-6 h-6 sm:w-8 sm:h-8 text-blue-400 shrink-0 ml-2" />
        </div>
      </div>
      ...
      <div class="bg-gray-800 rounded-lg p-3 sm:p-4">
        <div class="flex items-center justify-between">
          ...
          <AlertTriangle class="w-6 h-6 sm:w-8 sm:h-8 text-red-400 shrink-0 ml-2" />
        </div>
      </div>
      ...
      <div class="bg-gray-800 rounded-lg p-3 sm:p-4">
        <div class="flex items-center justify-between">
          ...
          <TrendingUp class="w-6 h-6 sm:w-8 sm:h-8 text-yellow-400 shrink-0 ml-2" />
        </div>
      </div>
      ...
      <div class="bg-gray-800 rounded-lg p-3 sm:p-4">
        <div class="flex items-center justify-between">
          ...
          <FileText class="w-6 h-6 sm:w-8 sm:h-8 text-green-400 shrink-0 ml-2" />
        </div>
      </div>
    </div>
    ...
    <div
      v-if="insights.length > 0"
      class="flex items-center space-x-1 sm:space-x-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-600"
    >
      <button
        v-for="category in categories"
        :key="category.id"
        :class="[
          'flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2 rounded-lg whitespace-nowrap transition-colors shrink-0',
    ...
    <div class="space-y-2 sm:space-y-3">
      ...
      >
        <div class="flex items-start space-x-2 sm:space-x-3">
          <component
            ...
            class="w-4 h-4 sm:w-5 sm:h-5 mt-0.5 shrink-0"
          />
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs sm:text-sm text-gray-300 wrap-break-word">
              {{ insight.description }}
            </p>
          </div>
        </div>
      ...
      <div
        v-if="filteredInsights.length === 0 && !isAnalyzing"
        class="text-center py-6 sm:py-8 text-gray-400"
      >
    ...
    <div v-if="recommendations.length > 0" class="bg-gray-800 rounded-lg p-4 sm:p-6">
      ...
      <div class="space-y-2 sm:space-y-3">
        ...
        >
          <span
            class="shrink-0 w-5 h-5 sm:w-6 sm:h-6 bg-yellow-600 text-white rounded-full flex items-center justify-center text-xs sm:text-sm font-medium"
          >
            {{ index + 1 }}
          </span>
          <p class="text-xs sm:text-sm text-gray-300 wrap-break-word">
