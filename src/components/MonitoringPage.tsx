  ...
  useEffect(() => {
    ...
    if (isMonitoring) {
      interval = setInterval(() => {
        ...
        if (newMetric.cpuUsage > 80) newAlerts.push(`High CPU usage: ${newMetric.cpuUsage.toFixed(1)}%`);
        if (newMetric.memoryUsage > 85) newAlerts.push(`High memory usage: ${newMetric.memoryUsage.toFixed(1)}%`);
        if (newMetric.diskUsage > 90) newAlerts.push(`High disk usage: ${newMetric.diskUsage.toFixed(1)}%`);
      ...
      <div className={styles.monitoringSection}>
        ...
        <div className={styles.metricsGrid}>
          <div className={styles.metricCard}>
            ...
            <div className={`${styles.metricTrend} ${getAlertLevel(latestMetric?.cpuUsage || 0, 70)}`}>
              {getAlertLevel(latestMetric?.cpuUsage || 0, 70) === 'critical' ? '⚠️ Critical' : 
               getAlertLevel(latestMetric?.cpuUsage || 0, 70) === 'warning' ? '⚠️ Warning' : '✅ Normal'}
            </div>
          ...
          <div className={styles.metricCard}>
            ...
            <div className={`${styles.metricTrend} ${getAlertLevel(latestMetric?.memoryUsage || 0, 80)}`}>
              {getAlertLevel(latestMetric?.memoryUsage || 0, 80) === 'critical' ? '⚠️ Critical' : 
               getAlertLevel(latestMetric?.memoryUsage || 0, 80) === 'warning' ? '⚠️ Warning' : '✅ Normal'}
            </div>
            <div className={styles.metricChart}>
              {metrics.slice(-12).map((metric, index) => (
                <div 
          ...
          <div className={styles.metricCard}>
            ...
            <div className={`${styles.metricTrend} ${getAlertLevel(latestMetric?.diskUsage || 0, 85)}`}>
              {getAlertLevel(latestMetric?.diskUsage || 0, 85) === 'critical' ? '⚠️ Critical' : 
               getAlertLevel(latestMetric?.diskUsage || 0, 85) === 'warning' ? '⚠️ Warning' : '✅ Normal'}
            </div>
            <div className={styles.metricChart}>
              {metrics.slice(-12).map((metric, index) => (
                <div 
          ...
          <div className={styles.metricCard}>
            ...
            <div className={`${styles.metricTrend} ${getAlertLevel(latestMetric?.networkUsage || 0, 70)}`}>
              {getAlertLevel(latestMetric?.networkUsage || 0, 70) === 'critical' ? '⚠️ Critical' : 
               getAlertLevel(latestMetric?.networkUsage || 0, 70) === 'warning' ? '⚠️ Warning' : '✅ Normal'}
            </div>
            <div className={styles.metricChart}>
              {metrics.slice(-12).map((metric, index) => (
                <div 
          ...
          <div className={styles.detailsSection}>
            ...
            <div className={styles.statusDetails}>
              ...
              <div className={styles.statusItem}>
                ...
                <span className={styles.statusValue}>{latestMetric?.timestamp.toLocaleString()}</span>
              </div>
              <div className={styles.statusItem}>
