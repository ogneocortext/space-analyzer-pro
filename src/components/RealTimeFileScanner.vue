<template>
  <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700 overflow-hidden">
    ...
    >
      <div v-if="uniqueScannedFiles.length === 0 && progress.files === 0" class="text-center text-slate-500 text-sm py-8">
        Waiting for scan to start...
      </div>
      <div v-else-if="uniqueScannedFiles.length === 0 && progress.files > 0" class="text-center text-slate-400 text-sm py-8">
      ...
      <div
        v-else
        v-for="(file, index) in uniqueScannedFiles"
        :key="`${file.path}-${index}`"
...
watch(() => props.scannedFiles.length, async () => {
...
const metrics = computed(() => {
  ...
  const totalSize = props.scannedFiles.reduce((acc, file) => acc + file.size, 0)
  ...
  props.scannedFiles.forEach(file => {
...
const uniqueScannedFiles = computed(() => {
  const seen = new Set()
