...
const Treemap: React.FC<TreemapProps> = ({ data, width, height, onNodeClick }) => {
  ...
  useEffect(() => {
    ...
    nodes.append('rect')
      ...
      .on('mouseover', function(event, d) {
        d3.select(this)
          .transition()
          .duration(200)
          .attr('fill', d3.color(color(d.data.category || 'Other') as string)?.darker(0.5)?.toString() || '#000');
        ...
        const tooltip = d3.select('body').append('div')
          ...
          .style('z-index', '1000')
          .html(`<strong>${d.data.name}</strong><br>Size: ${(d.value || 0).toLocaleString()} bytes<br>Category: ${d.data.category || 'Other'}`);
    ...
    nodes.filter(d => (d.x1 - d.x0) > 50 && (d.y1 - d.y0) > 20)
      .append('text')
      .attr('x', 4)
      .attr('y', 14)
      .attr('font-size', '10px')
