...
export const CodeMapVisualization: FC<CodeMapVisualizationProps> = ({
        ...
        for (const [filePath, analysis] of analyses) {
          ...
          for (const imp of analysis.imports) {
            const targetNode = nodes.find(
              (node) => node.name === imp.source || node.id.includes(imp.source)
            );
  ...
  useEffect(() => {
    ...
    // Create simulation
    const simulation = d3
      .forceSimulation<CodeMapNode>(data.nodes)
      .forceLink(
        d3
          .forceLink<CodeMapLink, CodeMapNode>(data.links)
          .id((d) => d.id)
          .distance(100)
          .strength(0.5)
      )
    ...
    const link = svg
      ...
      .attr("stroke-width", (d) =>
        d.strength === "strong" ? 2 : d.strength === "medium" ? 1.5 : 1
      )
    ...
    <div className="code-map-visualization">
      ...
      <div className="code-map-header">
        ...
        <div className="header-right">
          ...
          {/* Export */}
          <button onClick={handleExport} className="export-btn" title="Export as SVG">
            <Download size={16} />
          </button>
        </div>
      </div>
