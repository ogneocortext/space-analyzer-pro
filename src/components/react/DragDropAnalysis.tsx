...
const DragDropAnalysis: FC<DragDropProps> = ({
  ...
  const validateFiles = useCallback((files: FileList): DraggedFile[] => {
    ...
    Array.from(files).forEach(file => {
      ...
      if (file.size > maxFileSize) {
        errors.push(`${file.name} is too large (${formatFileSize(file.size)} > ${formatFileSize(maxFileSize)})`);
        return;
      }
  ...
  const analyzeFiles = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < draggedFiles.length; i++) {
        ...
        // Update file status
        setDraggedFiles(prev => prev.map(f => 
  ...
  return (
    <div className={`drag-drop-container ${className} ${isDragOver ? 'drag-over' : ''} ${disabled ? 'disabled' : ''}`}>
      {/* Hidden file input */}
      <input
        ref={fileInputRef}
        type="file"
            ...
            {draggedFiles.map((draggedFile) => (
              <div key={draggedFile.id} className={`file-item ${draggedFile.status}`}>
                ...
                <div className="file-status">
                  {draggedFile.status === 'pending' && (
                    <div className="status-pending">Ready</div>
                  )}
                ...
                {!isAnalyzing && (
                  <button
                    onClick={() => removeFile(draggedFile.id)}
