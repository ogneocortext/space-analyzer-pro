...
// @ts-ignore
import { OptimizedPieChart, OptimizedLineChart, OptimizedRadarChart } from './charts/OptimizedCharts';
// @ts-ignore
...
const SpaceAnalyzerDashboard: FC<SpaceAnalyzerDashboardProps> = ({ analysisResults, onNavigate }) => {
  ...
  const [notifications, setNotifications] = useState<Array<{id: string, type: 'info' | 'success' | 'warning', message: string, timestamp: Date}>>([]);
  ...
  useEffect(() => {
    ...
    if (!hasSeenWelcome) {
      setTimeout(() => {
        addNotification('success', '🎉 Welcome to Space Analyzer Pro! Use Ctrl+? for keyboard shortcuts.');
    ...
    <div className="flex-1 bg-slate-900 text-white overflow-y-auto scroll-smooth dashboard-container">
      ...
      >
        ...
        <div className="dashboard-grid mb-8">
          ...
          <div className="metric-card" data-tooltip="AI model accuracy and confidence score">
            ...
            <div className="metric-label responsive-text-xs flex items-center gap-1">
              ...
              <Eye className="w-3 h-3 text-slate-400" title="AI analysis based on file patterns and metadata" />
            </div>
            {/* Trust indicator bar */}
          ...
          <div className="dashboard-section">
            ...
            <div className="text-center mb-4">
              ...
              <div className="responsive-text-xs text-slate-500">
                Current: {totalSizeGB.toFixed(1)} GB → Projected: {(totalSizeGB * 1.125).toFixed(1)} GB
              </div>
            </div>
        ...
        <div className="dashboard-section mb-8">
          ...
          <p className="text-slate-400 mb-4 text-sm">Automated recommendations from Backend Analysis</p>
          ...
          <div className="space-y-4">
            {analysisResults.ai_insights?.storage_warnings?.slice(0, 2).map((warning, i) => (
              <div key={i} className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="bg-yellow-500/20 text-yellow-300 px-3 py-1 rounded-full text-sm">Medium Impact</span>
                </div>
            ...
            {analysisResults.ai_insights?.optimization_suggestions?.slice(0, 2).map((suggestion, i) => (
              <div key={i} className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full text-sm">Low Impact</span>
                </div>
            ...
            {(!analysisResults.ai_insights?.storage_warnings?.length && !analysisResults.ai_insights?.optimization_suggestions?.length) && (
              <div className="text-slate-400 italic">No specific insights generated for this scan.</div>
            )}
          </div>
        </div>
