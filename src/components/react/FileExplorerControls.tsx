  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
      <div className="flex flex-wrap gap-4 items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20"
          >
            <option value={SORT_TYPES.NAME} className="bg-gray-800">Name</option>
            <option value={SORT_TYPES.SIZE} className="bg-gray-800">Size</option>
            <option value={SORT_TYPES.CATEGORY} className="bg-gray-800">Category</option>
          </select>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-gray-400">
        <span>Showing {filteredCount.toLocaleString()} of {totalFiles.toLocaleString()} files</span>
        <span>{selectedCategory !== FILE_CATEGORIES.ALL ? `Filtered by ${selectedCategory}` : 'All categories'}</span>
      </div>
    </div>
  );
};