...
export const CodeAnalysisDisplay: FC<CodeAnalysisDisplayProps> = ({
  ...
  const getSeverityIcon = (severity: "error" | "warning" | "info") => {
    switch (severity) {
      case "error":
        return <XCircle size={14} className="severity-icon severity-icon-error" />;
      case "warning":
        return <AlertTriangle size={14} className="severity-icon severity-icon-warning" />;
      case "info":
        return <Info size={14} className="severity-icon severity-icon-info" />;
      default:
      ...
      <div className="code-analysis-panel">
        <div className="code-analysis-header">
          <div className="code-analysis-title">
            ...
            <p className="code-analysis-no-data-text">
              Run an analysis on a development directory to see code health metrics
            </p>
          </div>
        </div>
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          ...
          <div className="health-metric-item">
            ...
            <div className="health-metric-value">
              <div
                ...
                style={{ color: getHealthColor(healthMetrics.healthScore) }}
              >
      ...
      <div className="content-container">
        ...
        {selectedCategory === "unused" && (
          <UnusedCodeTab dependencyAnalysis={dependencyAnalysis} onFixIssue={onFixIssue} />
        )}

        {selectedCategory === "bugs" && (
          <BugsTab dependencyAnalysis={dependencyAnalysis} onFixIssue={onFixIssue} />
        )}
      </div>
...
const OverviewTab: FC<{
  healthMetrics: ProjectHealthMetrics | null;
  dependencyAnalysis: DependencyHealthAnalysis | null;
}> = ({ healthMetrics, dependencyAnalysis }) => {
      ...
      <div className="glass-panel">
        ...
        <div className="health-trend">
          ...
          <div className="health-description">
            {healthMetrics.healthScore >= 80
              ? "Your code is well-maintained with minimal issues"
              : healthMetrics.healthScore >= 60
                ? "Good code quality with some room for improvement"
                : healthMetrics.healthScore >= 40
                  ? "Code needs attention and cleanup"
    ...
    <div className="dependencies-tab">
      ...
      {missingDependencies.length > 0 && (
        <div className="missing-dependencies">
          <h4 className="missing-dependencies-title">
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      {deadCode.length > 0 && (
        <div
          className="glass-panel"
          style={{ background: "rgba(107, 114, 128, 0.1)", borderLeft: "4px solid #6b7280" }}
        >
          <h4
            style={{
              margin: "0 0 1rem 0",
