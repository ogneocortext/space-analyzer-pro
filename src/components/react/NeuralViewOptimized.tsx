...
const NeuralViewOptimized: FC<NeuralViewOptimizedProps> = ({
  ...
  const renderNeuralNetwork = useCallback(() => {
      ...
      neurons.forEach((neuron, i) => {
        neuron.connections.forEach((connId) => {
          ...
          if (other) {
            ...
            if (distance < 200) {
              const connection = optimizedData?.connections.find(
                (c) =>
    ...
    neurons.forEach((neuron) => {
      ...
      const baseSize =
        neuron.type === "file" && neuron.fileSize
          ? // @ts-ignore - fileSize property
      ...
      if (glowIntensity > 0) {
        ...
        glowGradient.addColorStop(
          0,
          `${glowColor}${Math.floor(glowIntensity * 200)
            .toString(16)
            .padStart(2, "0")}`
        );
        glowGradient.addColorStop(
          0.5,
          `${glowColor}${Math.floor(glowIntensity * 50)
            .toString(16)
            .padStart(2, "0")}`
        );
    ...
    if (hoveredNode && mousePos && screenSize.isDesktop) {
      ...
      if (hoveredNeuron) {
        ...
        // @ts-ignore - fileSize property
        const fileSize =
          hoveredNeuron.type === "file" && hoveredNeuron.fileSize
            ? // @ts-ignore - fileSize property
              `${(hoveredNeuron.fileSize / 1024 / 1024).toFixed(2)} MB`
            : hoveredNeuron.type === "directory"
              ? "Directory"
              : "File";
  ...
  const animate = useCallback(() => {
    ...
    renderNeuralNetwork,
    performance,
    optimizedData?.connections.length,
  ]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (newX !== activeNeuron.x || newY !== activeNeuron.y) {
        ...
        setNeurons((prev) =>
          prev.map((neuron) =>
            neuron.id === activeNeuron.id ? { ...neuron, x: newX, y: newY, pinned: true } : neuron
          )
        );
      }
    };
  ...
  const handleSimulationControl = useCallback(
    (action: "toggle" | "reset") => {
      if (action === "toggle") {
  ...
  const handleToggleConnections = useCallback(
    (enabled: boolean) => {
      ...
      accessibilityActions.announce(
        enabled ? "Connection lines enabled" : "Connection lines disabled"
      );
    },
    [accessibilityActions]
  );
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl max-w-2xl w-full p-6 shadow-2xl relative">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    ...
                    <span>
                      <strong>Blue Nodes:</strong> Directories & Folder Clusters
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-3 h-3 rounded-full bg-purple-500 block"></span>
                    <span>
                      <strong>Purple Nodes:</strong> Individual Files (Scale = Size)
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-8 h-px bg-blue-400/50 block"></span>
                    <span>
                      <strong>Lines:</strong> File Dependencies & Relationships
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    <Play size={16} />
                    <span>
                      <strong>Play/Pause:</strong> Control the neural simulation
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <RefreshCw size={16} />
                    <span>
                      <strong>Reset:</strong> Regenerate node positions
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <Activity size={16} />
                    <span>
                      <strong>Modes:</strong> Switch between Network, Heatmap, etc.
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-700">
              <p className="text-sm text-slate-400">
                The Neural View uses a physics-based force-directed graph to visualize your file
                system. Files that are semantically related or in the same directory will cluster
                together. Large files appear as larger nodes with stronger gravitational pull.
              </p>
            </div>
          </div>
        </div>
