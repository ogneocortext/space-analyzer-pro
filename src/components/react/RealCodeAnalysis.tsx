...
export const RealCodeAnalysis: React.FC<{ analysisData: AnalysisResult }> = ({ analysisData }) => {
  ...
  const codeAnalysis = useMemo(() => {
    ...
    const codeFiles = analysisData.files.filter(f => 
      ['js', 'jsx', 'ts', 'tsx', 'py', 'java', 'cpp', 'c', 'h', 'cs', 'go', 'rs'].includes(f.extension)
    );
    ...
    const packageFiles = codeFiles.filter(f => 
      f.name === 'package.json' || f.name === 'requirements.txt' || f.name === 'Cargo.toml' || f.name === 'go.mod'
    );
  ...
  const getIssueIcon = (type: string) => {
    switch (type) {
      ...
      case 'suggestion': return <Zap size={16} className="text-purple-400" />;
      default: return <Bug size={16} className="text-slate-400" />;
    }
  };
  ...
  const getDependencyStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'outdated': return <AlertTriangle size={16} className="text-yellow-400" />;
      case 'vulnerable': return <XCircle size={16} className="text-red-400" />;
      case 'missing': return <Info size={16} className="text-slate-400" />;
    ...
    <div className="p-8 text-white">
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6 mb-6">
        ...
        <div className="flex items-center gap-8">
          <div className="text-6xl font-bold" style={{ color: getHealthColor(codeAnalysis.metrics.maintainabilityIndex) }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg">
              {codeAnalysis.metrics.maintainabilityIndex >= 80 ? 'Excellent' :
               codeAnalysis.metrics.maintainabilityIndex >= 60 ? 'Good' :
               codeAnalysis.metrics.maintainabilityIndex >= 40 ? 'Fair' : 'Poor'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
          ...
          <div className="text-3xl font-bold text-cyan-400 mb-2">{codeAnalysis.metrics.totalFiles}</div>
            ...
            {filteredIssues.length === 0 ? (
              ...
              filteredIssues.map(issue => (
                <div
                  ...
                  style={{ borderColor: issue.severity === 'high' ? '#ef4444' : issue.severity === 'medium' ? '#f59e0b' : '#6b7280' }}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-2">
                        <span className="text-xs bg-slate-600 px-2 py-1 rounded">{issue.category}</span>
            ...
            {codeAnalysis.dependencies.length === 0 ? (
              ...
              codeAnalysis.dependencies.map((dep, index) => (
                ...
                >
                  ...
                  <span className={`text-xs px-2 py-1 rounded ${
                    dep.status === 'healthy' ? 'bg-green-500/20 text-green-400' :
                    dep.status === 'outdated' ? 'bg-yellow-500/20 text-yellow-400' :
                    dep.status === 'vulnerable' ? 'bg-red-500/20 text-red-400' :
                    'bg-slate-600 text-slate-400'
                  }`}>
                    {dep.status}
                  </span>
