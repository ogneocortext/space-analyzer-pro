                ...
                {data.map((item, index) => {
                  ...
                  const previousAngles = data.slice(0, index).reduce((sum, d) => {
                    const prevPercentage =
                      (d.value / data.reduce((sum, d) => sum + d.value, 0)) * 100;
    ...
    <div className={`chart-container polished-card ${className}`} ref={chartRef}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <div className="p-2 bg-blue-500/20 rounded-lg text-blue-400">{getChartIcon()}</div>
          <div>
        ...
        <div className="chart-legend">
          {data.map((item) => (
            <div key={item.name} className="legend-item">
              <div className="legend-color" style={{ backgroundColor: item.color || "#3b82f6" }} />
              <span>{item.name}</span>
