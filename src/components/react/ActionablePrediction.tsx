...
const ActionablePrediction: FC<ActionablePredictionProps> = ({
  ...
  const executeAction = async (action: string, target: string, insight: PredictiveInsight) => {
    ...
    try {
      ...
      if (
        dependencyAnalysis &&
        (dependencyAnalysis.overallRisk === "critical" || dependencyAnalysis.overallRisk === "high")
      ) {
    ...
    switch (actualAction) {
      ...
      case "organize":
        await executeOrganization(
          dependencyAnalysis?.targetFiles[0]?.split("/")[0] || "",
          {} as any
        );
        break;
      case "archive":
  ...
  if (!insights || insights.length === 0) {
    return (
      <div className="actionable-prediction-container">
        ...
        <p className="actionable-no-insights-text">
        ...
        <div className="actionable-modal-overlay">
          <div className="actionable-modal">
            <div className="actionable-modal-header">
              ...
              <button
                onClick={() => setShowDependencyAnalysis(false)}
                className="actionable-close-btn"
        ...
        return (
          <div key={index} className="glass-panel actionable-category-panel">
            ...
            <div className="actionable-category-header">
              ...
              <h4 className="actionable-category-title">
              ...
              <div className="actionable-category-stats">
                ...
                <div className="actionable-stat-item">
                  ...
                  <span className="actionable-stat-value">
                    {(categoryFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(1)}MB
                  </span>
                </div>
                <div className="actionable-stat-item">
                ...
                {recommendedActions.map((action, actionIndex) => (
                  <div key={actionIndex} className="actionable-action-item">
                    ...
                    <div className="actionable-action-priority">
                      <span className={`actionable-priority-${action.priority}`}>
                        {action.priority}
                      </span>
                      <button
                        onClick={() => executeAction(action.type, category, insight)}
                        disabled={
                          selectedAction === `${action.type}-${category}` || analyzingDependencies
                        }
                        style={{
                          padding: "0.5rem 1rem",
                          borderRadius: "6px",
                          background:
                            selectedAction === `${action.type}-${category}` ? "#6b7280" : "#3b82f6",
                          color: "white",
                          border: "none",
                          cursor:
                            selectedAction === `${action.type}-${category}` || analyzingDependencies
