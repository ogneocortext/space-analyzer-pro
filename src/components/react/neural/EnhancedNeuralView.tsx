...
const EnhancedNeuralView: React.FC<EnhancedNeuralViewProps> = ({
  ...
  const processedData = useMemo(() => {
    ...
    const filteredConnections = data.connections.filter(conn => {
      ...
      ...conn,
      visible: filteredNodes.some(n => n.id === conn.from) && filteredNodes.some(n => n.id === conn.to)
    }));
  ...
  const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const clickedNode = processedData.nodes.find(node => {
      if (node.type === 'directory') {
        return x >= node.x - node.size && x <= node.x + node.size &&
               y >= node.y - node.size && y <= node.y + node.size;
      } else {
            ...
            <div className={styles.helpContent}>
              ...
              <div className={styles.helpSections}>
                ...
                <div className={styles.helpSection}>
                  <h4>Node Types</h4>
                  <ul>
                    <li><span className={styles.nodeIndicatorDirectory}></span> Directories</li>
                    <li><span className={styles.nodeIndicatorFile}></span> Files</li>
                  </ul>
                </div>
              </div>
            </div>
