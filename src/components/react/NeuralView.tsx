...
import {
  convertDependencyGraphToNeuralData,
  type DependencyGraph,
} from "../utils/dependencyGraphConverter";
...
const NeuralView: FC<NeuralViewProps> = ({ data, dependencyGraph, isLoading, error }) => {
  ...
  const convertedData = dependencyGraph
    ? convertDependencyGraphToNeuralData(dependencyGraph)
    : null;
      ...
      if (node.type === "directory") {
        ...
        data.nodes.forEach((fileNode) => {
          if (!processed.has(fileNode.id) && fileNode.type === "file") {
            ...
            if (
              fileNode.id.startsWith(directoryPath + "/") ||
        ...
        neuron.connections.forEach((connId) => {
          ...
          if (other) {
            ...
            if (distance < 200) {
              ...
              const connection = safeData.connections.find(
                (c) =>
    ...
    <div className={styles.neuralViewContainer} style={{ position: "relative" }}>
      ...
      <>
        ...
        >
          ...
          {error && <div className={styles.neuralError}>Neural analysis error: {error}</div>}
