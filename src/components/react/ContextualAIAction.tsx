...
export const ContextualAIAction: FC<ContextualAIActionProps> = ({
  ...
  return (
    <div className="relative inline-block">
      ...
      {isExpanded && (
        <div
          className={`absolute z-50 ${getPositionClasses()} w-64 p-2 rounded-xl border backdrop-blur-md shadow-2xl bg-slate-900/95 border-white/20`}
        >
          ...
          {/* Tooltip Arrow */}
          <div
            className={`absolute w-2 h-2 bg-slate-900/95 border rotate-45 ${
              position === "top"
                ? "top-full left-1/2 -translate-x-1/2 -mt-1 border-l border-t border-white/20"
                : position === "bottom"
                  ? "bottom-full left-1/2 -translate-x-1/2 -mb-1 border-r border-b border-white/20"
                  : position === "left"
                    ? "left-full top-1/2 -translate-y-1/2 -ml-1 border-t border-r border-white/20"
      ...
      {isHovered && !isExpanded && (
        <div
          className={`absolute z-40 ${getPositionClasses()} px-3 py-2 rounded-lg border backdrop-blur-md shadow-lg bg-slate-900/95 border-white/20 text-sm text-gray-200 whitespace-nowrap`}
        >
          Ask AI about {contextType}
          <div
            className={`absolute w-2 h-2 bg-slate-900/95 border rotate-45 ${
