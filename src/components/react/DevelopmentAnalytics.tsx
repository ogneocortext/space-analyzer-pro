...
const DevelopmentAnalytics: React.FC<DevelopmentAnalyticsProps> = ({ projectPath }) => {
  const [activeTab, setActiveTab] = useState<"duplication" | "performance" | "dependencies">(
    "duplication"
  );
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  const { detectCodeDuplication, trackBuildPerformance, analyzeDependencies } =
    useDevelopmentAnalytics(projectPath);
  ...
  const renderCodeDuplication = () => {
    ...
    const similarityData =
      duplicationData.similarFiles?.map((file: any, index: number) => ({
        name: `File ${index + 1}`,
    ...
    const duplicationDistribution = [
      ...
      {
        name: "80-100%",
        value: duplicationData.duplicationRanges?.critical || 0,
        color: "#dc2626",
      },
    ];
    ...
    return (
      <div className="analytics-section">
        <div className="section-header">
          <h3>
            <Code2 size={20} /> Code Duplication Analysis
          </h3>
          <div className="metrics-summary">
            <div className="metric">
              <span className="metric-value">
                {duplicationData.overallDuplication?.toFixed(1) || 0}%
              </span>
        ...
        {duplicationData.recommendations?.length > 0 && (
          <div className="recommendations">
            <h4>
              <Target size={16} /> Optimization Recommendations
            </h4>
            <ul>
  ...
  const renderBuildPerformance = () => {
    ...
    const buildTimeData =
      performanceData.buildHistory?.map((build: any, index: number) => ({
        build: `Build ${index + 1}`,
    ...
    const performanceMetrics = [
      {
        name: "Build Time",
        value: performanceData.averageBuildTime / 1000,
        unit: "s",
        color: "#3b82f6",
      },
      {
        name: "Bundle Size",
        value: performanceData.averageBundleSize / (1024 * 1024),
        unit: "MB",
        color: "#10b981",
      },
      {
        name: "Optimization",
        value: performanceData.optimizationScore,
        unit: "%",
        color: "#f59e0b",
      },
    ];
    ...
    return (
      <div className="analytics-section">
        <div className="section-header">
          <h3>
            <Zap size={20} /> Build Performance Analysis
          </h3>
          <div className="metrics-summary">
            <div className="metric">
              <span className="metric-value">
                {(performanceData.averageBuildTime / 1000).toFixed(1)}s
              </span>
              <span className="metric-label">Avg Build Time</span>
            </div>
            <div className="metric">
              <span className="metric-value">
                {(performanceData.averageBundleSize / (1024 * 1024)).toFixed(1)}MB
              </span>
        ...
        {performanceData.recommendations?.length > 0 && (
          <div className="recommendations">
            <h4>
              <TrendingUp size={16} /> Performance Improvements
            </h4>
            <ul>
  ...
  const renderDependencyAnalysis = () => {
    ...
    const dependencyTypes = [
      ...
      {
        name: "Outdated",
        value: dependencyData.outdatedDependencies?.length || 0,
        color: "#f59e0b",
      },
    ];

    const securityIssues =
      dependencyData.securityIssues?.map((issue: any, index: number) => ({
        name: `Issue ${index + 1}`,
    ...
    return (
      <div className="analytics-section">
        <div className="section-header">
          <h3>
            <BarChart3 size={20} /> Dependency Analysis
          </h3>
          <div className="metrics-summary">
            <div className="metric">
        ...
        {dependencyData.recommendations?.length > 0 && (
          <div className="recommendations">
            <h4>
              <AlertTriangle size={16} /> Dependency Optimization
            </h4>
            <ul>
