...
const EnhancedFileBrowser: React.FC<EnhancedFileBrowserProps> = ({
  ...
  const getFileIconComponent = useCallback(
    (file: FileItem) => {
    ...
    <div className="enhanced-file-browser" ref={containerRef}>
      ...
      <div className="file-browser-controls">
        ...
        <div className="selection-controls">
          <button onClick={selectAllFiles} className="select-all-button" title="Select all files">
            <CheckSquare size={16} />
          </button>
          <button
      ...
      <div className="file-list-container">
        {processedFiles.length === 0 ? (
          <div className="empty-state">
            ...
            <p>{searchQuery ? "No files match your search criteria" : "This folder is empty"}</p>
          </div>
        ) : (
          <div className={`file-list ${viewMode}`}>
            {processedFiles.map((file) => (
              ...
              >
                ...
                <div className="file-icon">{getFileIconComponent(file)}</div>

                <div className="file-info">
                  <div className="file-name">{file.name}</div>
                  <div className="file-meta">
                    <span className="file-size">{formatFileSize(file.size)}</span>
                    {file.extension && <span className="file-extension">.{file.extension}</span>}
                ...
                {viewMode === "tree" &&
                  ...
                  file.children.length > 0 && (
                    ...
                    >
                      <ChevronDown
                        size={16}
                        className={`expand-icon ${expandedFolders.has(file.id) ? "expanded" : ""}`}
                      />
                    </button>
                  )}
              </motion.div>
