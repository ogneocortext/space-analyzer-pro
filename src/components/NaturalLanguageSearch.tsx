...
import { Search, Brain, Sparkles, Clock, FileText, HardDrive, Calendar, Filter } from 'lucide-react';
...
const NaturalLanguageSearch: React.FC<NaturalLanguageSearchProps> = ({
  ...
  const parseQuery = useCallback(async (text: string): Promise<NLQuery> => {
    ...
    if (lowerText.includes('show') || lowerText.includes('find') || lowerText.includes('search')) {
      ...
      intent = 'filter';
    } else if (lowerText.includes('analyze') || lowerText.includes('what') || lowerText.includes('how much')) {
      intent = 'analyze';
    ...
    const datePatterns = [
      ...
      { pattern: /yesterday/i, value: new Date(Date.now() - 24 * 60 * 60 * 1000), operator: '=' as const },
      { pattern: /last week/i, value: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), operator: '>=' as const },
      { pattern: /last month/i, value: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), operator: '>=' as const },
      { pattern: /last (\d+) days?/i, value: new Date(Date.now() - parseInt(RegExp.$1) * 24 * 60 * 60 * 1000), operator: '>=' as const },
      { pattern: /older than (\d+) months?/i, value: new Date(Date.now() - parseInt(RegExp.$1) * 30 * 24 * 60 * 60 * 1000), operator: '<=' as const },
      { pattern: /older than (\d+) weeks?/i, value: new Date(Date.now() - parseInt(RegExp.$1) * 7 * 24 * 60 * 60 * 1000), operator: '<=' as const },
      { pattern: /older than (\d+) days?/i, value: new Date(Date.now() - parseInt(RegExp.$1) * 24 * 60 * 60 * 1000), operator: '<=' as const },
      { pattern: /recent/i, value: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), operator: '>=' as const }
    ];
    ...
    const typePatterns = [
      { pattern: /\b(pdf|doc|docx|txt|jpg|jpeg|png|gif|mp4|avi|mov|mp3|wav|zip|rar|exe|dmg)\b/i, type: 'extension' as const },
      { pattern: /\b(image|video|audio|document|text|archive|executable)\b/i, type: 'category' as const },
      { pattern: /\b(pictures?|photos?|videos?|music|documents?|files?)\b/i, type: 'category' as const }
    ];
    ...
    if (entities.length > 0) {
      generatedQuery = entities.map(entity => {
        switch (entity.type) {
          case 'size':
    ...
    <div className={`relative ${className}`}>
      ...
      <AnimatePresence>
        {input && (
          ...
          >
            Type natural language queries like "show me files larger than 100MB" or "find recent videos"
          </motion.div>
        )}
      </AnimatePresence>
    </div>
