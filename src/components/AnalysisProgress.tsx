...
const AnalysisProgress: FC<AnalysisProgressProps> = ({
    ...
    const getMeaningfulStatus = () => {
        ...
        if (status && (status.includes('🔍') || status.includes('🧠') || status.includes('🤖') || status.includes('💡') || status.includes('📊') || status.includes('✅'))) {
            return status;
        }
        
        // If status is from real progress callback, use it
        if (status && (status.includes('Scanning:') || status.includes('Analyzing:') || status.includes('Found') || status.includes('Generating') || status.includes('Finalizing'))) {
            return status;
        }
    ...
    return (
        <div className={`bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-4 sm:p-6 shadow-2xl ${className}`}>
            {/* Header */}
                ...
                <div className="space-y-2">
                    {ANALYSIS_STAGES.map((stage, index) => {
                        const stageProgress = Math.min(100, Math.max(0,
                            ((progress / 100) * ANALYSIS_STAGES.length - index) * 100
                        ));
