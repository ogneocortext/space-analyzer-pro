        ...
        <div class="performance-grid">
          <div class="metric-card">
            <div class="metric-header">
              ...
              <span class="metric-value">{{ Math.round(performanceMetrics.acceptanceRate * 100) }}%</span>
            </div>
          ...
          <div class="metric-card">
            <div class="metric-header">
              ...
              <span class="metric-value">{{ Math.round(performanceMetrics.conversionRate * 100) }}%</span>
            </div>
        ...
        <div class="learning-rate-grid">
          ...
          <div class="rate-metrics">
            ...
            <div class="rate-metric">
              ...
              <span class="rate-value">{{ learningRateTrend > 0 ? '↑' : '↓' }} {{ Math.abs(learningRateTrend).toFixed(2) }}%</span>
            </div>
            <div class="rate-metric">
        ...
        <div class="behavior-visualization">
          <div class="activity-heatmap">
            ...
            <div class="heatmap-legend">
              ...
              <div class="legend-colors">
                <div v-for="i in 5" :key="i" class="legend-color" :style="{ backgroundColor: getHeatmapColor(i - 1) }"></div>
              </div>
          ...
          <div class="behavior-metrics">
            <div class="behavior-metric-card">
              ...
              <div class="circular-progress" :style="{ '--progress': behaviorMetrics.consistencyScore }">
                <span class="progress-value">{{ Math.round(behaviorMetrics.consistencyScore * 100) }}%</span>
              </div>
            </div>
            <div class="behavior-metric-card">
              <h6>Exploration Rate</h6>
              <div class="circular-progress" :style="{ '--progress': behaviorMetrics.explorationRate }">
                <span class="progress-value">{{ Math.round(behaviorMetrics.explorationRate * 100) }}%</span>
              </div>
            </div>
            <div class="behavior-metric-card">
...
const loadLearningRateData = async () => {
  const adjustments = await indexedDBPersistence.loadAnalyticsData('learning-rate-adjustment', 1000)
...
const renderPerformanceCharts = () => {
  renderMiniChart(acceptanceChart.value, 'line', generateMockData(24, performanceMetrics.value.acceptanceRate))
  renderMiniChart(responseTimeChart.value, 'bar', generateMockData(24, performanceMetrics.value.avgResponseTime))
  renderMiniChart(satisfactionChart.value, 'area', generateMockData(24, performanceMetrics.value.satisfaction))
}
...
const renderRatingChart = () => {
  ...
  const counts = ratings.map(rating => 
    feedbackCategories.value.find(c => c.name.includes(rating.toString()))?.count || 0
  )
...
const calculateAvgResponseTime = (feedback: any[]): number => {
  const responseTimes = feedback.map(f => f.feedback?.responseTime).filter(t => t !== undefined)
  return responseTimes.length > 0 ? responseTimes.reduce((sum, t) => sum + t, 0) / responseTimes.length : 0
}
...
const calculateEfficiencyScore = (adjustments: any[]): number => {
  ...
  const improvementCount = recent.filter((adj, i) => 
    i > 0 && adj.adjustment.rate > recent[i - 1].adjustment.rate
  ).length
