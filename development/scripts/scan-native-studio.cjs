...
async function scanDirectory() {
    ...
    try {
        ...
        console.log(`   Total Size: ${formatBytes(response.data.data.totalSize)}`);
        console.log(`   Analysis Time: ${response.data.data.analysisTime}ms`);
        
        console.log('\n📁 Categories:');
        Object.entries(response.data.data.categories).forEach(([category, stats]) => {
            console.log(`   ${category}: ${stats.count} files (${formatBytes(stats.size)})`);
        });
        
        console.log('\n💡 AI Insights:');
        if (response.data.data.ai_insights && response.data.data.ai_insights.optimization_suggestions) {
            response.data.data.ai_insights.optimization_suggestions.forEach(suggestion => {
